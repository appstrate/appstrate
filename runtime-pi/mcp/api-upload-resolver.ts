...
/**
 * `McpApiUploadResolver` — orchestrates chunked file uploads over an
 * integration's existing `{ns}__api_call` MCP tool.
 *
 * Why agent-side, not a new sidecar tool: the chunking logic is purely
 * client-side state, and the file lives in the agent's workspace (the
 * sidecar has no workspace mount). The integration's `{ns}__api_call`
 * tool already does the right thing per chunk (credential injection,
 * `authorizedUris` check, response-header propagation via `_meta`).
 * Putting the orchestration here keeps the sidecar dumb (no workspace
 * mount, no upload-protocol awareness, no new attack surface) and the
 ...
 *      symlink refusal — same path safety as `api_call`).
...
export class McpApiUploadResolver {
