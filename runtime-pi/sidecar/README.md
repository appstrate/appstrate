...
The agent container has no platform credentials, no access to `host.docker.internal`, and neither `SIDECAR_URL` nor `SIDECAR_AUTH_TOKEN` in the environment of the process that runs the agent — [`launcher.ts`](../launcher.ts) hands **both** (and the sink credentials) to [`entrypoint.ts`](../entrypoint.ts) over stdin, and both processes are non-dumpable, so nothing the agent spawns can recover them from its environment or through `/proc`. The token goes with the URL rather than surviving it, and that is the point: together they are the capability to spend the org's provider credential through `/llm/*`. All credential-bearing capabilities are exposed to the agent LLM as typed Pi tools — never as a bare URL.
...
**The sidecar token is NOT the run token** and carries none of its authority. It is minted per run by the launcher and handed to both sides of the pair — the sidecar container's `SIDECAR_AUTH_TOKEN` and, via `buildRuntimePiEnv`, the agent container's — and it asserts exactly one thing: "I am the agent container talking to my own sidecar". The zero-knowledge boundary is unchanged: the agent still holds no token that can call the PLATFORM back, and this one cannot be used to derive one. It gets its own header rather than `Authorization` because on `/llm/*` that slot already carries the placeholder credential the sidecar replaces with its own upstream auth.

It exists because the per-run Docker network stopped being the boundary. `integration-runtime-adapter-docker.ts` attaches every third-party integration runner to the same bridge and hands it `http://sidecar:<port>`, so "on the network" no longer means "is the agent": without the token a `source.kind: "local"` integration reaches the LLM proxy with one `curl` and spends the run's model credential. Deny-by-default rather than per-route opt-in because `/llm/*`, `/mcp`, `/integrations/boot-report` and `/runtime-events` each ended up open one at a time; a route added later is now protected without anyone remembering to say so. Nothing runner-facing lives on this app — the per-integration egress / MITM listeners, the transparent plane, the DNS responder and the forward proxy are their own listeners; the CONNECT, transparent-splicer, MITM and forward-proxy listeners check the connecting peer's IP instead of a token, and the DNS responder answers anyone with the sidecar's IP, which grants nothing (#1458, `docs/architecture/SIDECAR.md` → "Runner egress allowlist") — so there is no exemption to carve for integration runners. They are not supposed to reach this app at all.

- `GET /health` — Readiness probe, and the only unauthenticated route. Returns 200 when ready, 503 (`{ status: "degraded" }`) otherwise.
- `ALL /llm/*` — LLM reverse proxy consumed by the in-container Pi SDK as `${MODEL_BASE_URL}/v1/chat/completions` (or equivalent). Two modes. `platform` (an API-key model): the sidecar holds no provider key and forwards the one inference call to the platform's metered LLM proxy (`/internal/llm-proxy/<apiShape>`), with the run token in place of the SDK's placeholder. `oauth` (a subscription): it swaps the placeholder bearer for a fresh subscription token and forwards to the provider, SSRF-blocked against private/metadata addresses. Both stream the upstream response back zero-copy, bound by `LLM_PROXY_TIMEOUT_MS`. Both forwarding paths strip `x-appstrate-sidecar-auth` and its `x-appstrate-pi-sdk` sibling from the header set, so the agent's sidecar token never rides on to a vendor.
...
| `LLM_PROXY_TIMEOUT_MS`       | 30 min | `/llm/*` HTTP passthrough timeout — long enough for streamed agentic completions (raised from 5 min, which was killing legitimate long-running agentic runs at exactly the 5-minute mark).                                                               |

`STREAMING_THRESHOLD` (1 MB, the `Content-Length` above which `api_call` switches to streaming) and `MAX_STREAMED_BODY_SIZE` (100 MB, the ceiling on streamed request and response bodies) used to be listed here as sidecar knobs. They are neither sidecar constants nor settable: both are compiled into the shared outbound-HTTP engine `@appstrate/afps-runtime/resolvers`, which is where the in-container resolvers and the platform's `/api/credential-proxy/proxy` route both read them. Changing either is a code change in that package.

When the token budget decides a response should not be returned inline, the bytes are stored in the run-scoped `BlobStore` (128 MiB cap in production — `RUN_BLOB_STORE_MAX_BYTES` in `app.ts`; the store takes no default, deliberately, since any cap at the sidecar's 256 MiB cgroup limit lets the kernel OOM-killer fire before the store's own guard. ULID URIs, traversal-safe) and the tool returns a `resource_link` block. The agent reads the bytes on demand via `client.readResource({ uri })`.
