...
export function createForwardProxy(deps: ForwardProxyDeps): ForwardProxyResult {
  ...
  function getUpstreamProxy(): { host: string; port: number; auth: string | null } | null {
    ...
    try {
      ...
      console.error(JSON.stringify({ msg: "Invalid proxy URL, ignoring", proxyUrl: config.proxyUrl }));
      return null;
    }
  }
  ...
  function relay(s1: Socket, s2: Socket): void {
    ...
    s1.setTimeout(SOCKET_IDLE_TIMEOUT_MS, () => { s1.destroy(); });
    s2.setTimeout(SOCKET_IDLE_TIMEOUT_MS, () => { s2.destroy(); });
    s1.on("error", () => s2.destroy());
    s2.on("error", () => s1.destroy());
    s1.on("close", () => { if (!s2.destroyed) s2.destroy(); });
    s2.on("close", () => { if (!s1.destroyed) s1.destroy(); });
  }

  /** Strip hop-by-hop headers + Connection-listed headers from incoming request. */
  function forwardHeaders(raw: IncomingMessage["headers"]): Record<string, string | string[] | undefined> {
  ...
  const server = createHttpServer((req: IncomingMessage, res: ServerResponse) => {
    ...
    proxyReq.on("error", (err) => {
      console.error(JSON.stringify({ msg: "Forward proxy HTTP error", target: targetUrl, error: err.message }));
    ...
    if (upstream) {
      ...
      const onData = (chunk: Buffer) => {
        ...
        if (bufferSize > MAX_CONNECT_HEADER_SIZE) {
          console.error(JSON.stringify({ msg: "CONNECT header too large from upstream", target, size: bufferSize }));
      ...
      proxySocket.on("error", (err) => {
        console.error(JSON.stringify({ msg: "CONNECT upstream error", target, error: err.message }));
        clientSocket.destroy();
      });
  ...
  return {
    server,
    ready: readyPromise,
    get readySync() { return readySyncFlag; },
    address: () => {
