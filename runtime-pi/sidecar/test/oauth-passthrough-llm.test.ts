...
/**
 * Integration coverage for the sidecar's `/llm/*` `oauth-passthrough` path —
 * the ToS-clean runner mode for a driver that signs its OWN provider
 * fingerprint (the official Claude Agent SDK binary).
 *
 * The defining property under test: the sidecar does NOT forge. Unlike the
 * `oauth` path (covered in `oauth-llm.test.ts`), there are no identity headers,
 * no `system`-prepend, no `forceStream`. It only swaps the bearer for a fresh
 * real token, ensures the OAuth beta is present, and forwards the driver's own
 * fingerprint untouched. Mocks the platform token endpoint and the upstream
 * provider via one `fetchFn` dispatcher.
 */
...
import type { CredentialsResponse, LlmProxyOauthPassthroughConfig } from "../helpers.ts";
...
const PASSTHROUGH: LlmProxyOauthPassthroughConfig = {
  authMode: "oauth-passthrough",
  baseUrl: "https://api.anthropic.com",
  credentialId: "conn-passthrough",
};
...
describe("/llm/* oauth-passthrough — no forging", () => {
  ...
  it("honours a custom oauthBeta flag", async () => {
    const { fetchFn, calls } = setupFetchMock(upstreamOk);
    const deps = makeDeps(fetchFn);
    deps.config.llm = { ...PASSTHROUGH, oauthBeta: "oauth-2099-01-01" };
    const app = createApp(deps);
