...
describe("createIntegrationCredentialsSource — refreshOnUnauthorizedDetailed (tri-state)", () => {
  function mk(fetchFn: typeof fetch, minRefreshIntervalMs = 0) {
  ...
  it("returns 'refreshed' on 200", async () => {
    const source = mk(
      (async () =>
        new Response(JSON.stringify(makeWireJson("tok-2")), {
          status: 200,
        })) as unknown as typeof fetch,
    );
    expect(await source.refreshOnUnauthorizedDetailed("primary")).toBe("refreshed");
  });

  it("returns 'terminal' on 410 (revoked / unrefreshable)", async () => {
    const source = mk((async () => new Response("", { status: 410 })) as unknown as typeof fetch);
    expect(await source.refreshOnUnauthorizedDetailed("primary")).toBe("terminal");
  });

  it("returns 'transient' on 5xx and on network failure", async () => {
    const s5xx = mk((async () => new Response("", { status: 502 })) as unknown as typeof fetch);
    expect(await s5xx.refreshOnUnauthorizedDetailed("primary")).toBe("transient");
    const sNet = mk((async () => {
      throw new TypeError("ConnectionRefused");
    }) as unknown as typeof fetch);
    expect(await sNet.refreshOnUnauthorizedDetailed("primary")).toBe("transient");
  });

  it("returns 'transient' on a cooldown-suppressed call", async () => {
    const source = mk(
      (async () => new Response("", { status: 410 })) as unknown as typeof fetch,
      60_000,
    );
    expect(await source.refreshOnUnauthorizedDetailed("primary")).toBe("terminal");
    // Within cooldown → suppressed → transient (NOT a fresh verdict).
    expect(await source.refreshOnUnauthorizedDetailed("primary")).toBe("transient");
  });

  it("re-login handler maps success→'refreshed', failure→'terminal'", async () => {
    const ok = mk((async () => new Response("", { status: 500 })) as unknown as typeof fetch, 0);
    ok.setReloginHandler("primary", async () => true, [401]);
    expect(await ok.refreshOnUnauthorizedDetailed("primary")).toBe("refreshed");

    const bad = mk((async () => new Response("", { status: 500 })) as unknown as typeof fetch, 0);
    bad.setReloginHandler("primary", async () => false, [401]);
    expect(await bad.refreshOnUnauthorizedDetailed("primary")).toBe("terminal");
  });
});

describe("createIntegrationCredentialsSource — reportAuthFailure", () => {
  it("POSTs the report-auth-failure endpoint with the bearer token", async () => {
    ...
    await source.reportAuthFailure("primary");
    expect(calls.length).toBe(1);
    expect(calls[0]!.method).toBe("POST");
    expect(calls[0]!.url).toBe(
      "http://api/internal/integration-credentials/@test/integ/report-auth-failure",
    );
    expect(calls[0]!.auth).toBe("Bearer run-tok");
  });

  it("reports at most once per source", async () => {
    let calls = 0;
    const fetchFn = (async () => {
      calls += 1;
      return new Response(null, { status: 204 });
    ...
    await source.reportAuthFailure("primary");
    await source.reportAuthFailure("primary");
    await source.reportAuthFailure("other");
    expect(calls).toBe(1);
  });

  it("never throws on a non-OK status or a network failure", async () => {
    const sBad = createIntegrationCredentialsSource({
      integrationId: "@test/integ",
      platformApiUrl: "http://api",
      runToken: "run-tok",
      initialPayload: makePayload("tok-1"),
    ...
    await sBad.reportAuthFailure("primary"); // resolves, no throw

    const sNet = createIntegrationCredentialsSource({
      integrationId: "@test/integ",
      platformApiUrl: "http://api",
      runToken: "run-tok",
      initialPayload: makePayload("tok-1"),
    ...
    await sNet.reportAuthFailure("primary"); // resolves, no throw
