 ...
 * and the impossible half-configured state (`RUN_ID` set, `SIDECAR_DNS_ALIAS`
 * missing) must throw rather than land there silently.
 *
...
describe("resolveRunPlane — impossible half-configured launcher", () => {
  it("RUN_ID without SIDECAR_DNS_ALIAS throws instead of degrading to unfiltered egress", () => {
    expect(() => resolveRunPlane(env({ RUN_ID: "run_abc123" }))).toThrow(
      /RUN_ID is set but SIDECAR_DNS_ALIAS is not/,
    );
  });

  it("RUN_ID with an empty SIDECAR_DNS_ALIAS throws too", () => {
    expect(() => resolveRunPlane(env({ RUN_ID: "run_abc123", SIDECAR_DNS_ALIAS: "" }))).toThrow(
      /RUN_ID is set but SIDECAR_DNS_ALIAS is not/,
    );
  });

  it("the error names the credential-exposure consequence, not just the missing var", () => {
    expect(() => resolveRunPlane(env({ RUN_ID: "run_abc123" }))).toThrow(/unfiltered egress/);
  });
});
