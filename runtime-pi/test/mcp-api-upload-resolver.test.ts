...
import { McpApiUploadResolver } from "../mcp/api-upload-resolver.ts";
 ...
 * resolver's `apiCallToolName` field carries this tool name (the integration
 * is implied by the tool name). The in-process mock registers a tool under
 * this exact name.
 */
...
describe("McpApiUploadResolver — google-resumable", () => {
...
describe("McpApiUploadResolver — s3-multipart", () => {
...
describe("McpApiUploadResolver — tus", () => {
...
describe("McpApiUploadResolver — ms-resumable", () => {
...
describe("McpApiUploadResolver — cross-cutting", () => {
  ...
  it("surfaces sidecar pre-flight errors with status 0 (not synthetic 502)", async () => {
    ...
    const tool: AppstrateToolDefinition = {
      descriptor: { name: API_CALL_TOOL, description: "mock", inputSchema: { type: "object" } },
      handler: (async () => ({
...
describe("McpApiUploadResolver — cancellation parity", () => {
...
describe("McpApiUploadResolver — s3-multipart ETag round-trip", () => {
