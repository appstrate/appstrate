...
- **Signup sans SMTP ≠ signup public** : le `allowSignup` du client OAuth reste la barrière primaire, sur tous les niveaux (instance/org/application) depuis le refactor `a2aae3af`. Un client `allowSignup: false` refuse l'inscription même si SMTP absent — pas de régression. L'UI login cache le CTA "Créer un compte" et `/api/oauth/register` renvoie "Inscription fermée" (alignement SOTA FusionAuth/Auth0/Okta : pas de show-and-reject).
