...
| TypeScript     | **TS 6 + TS 7 side-by-side**. `tsc` resolves to `@typescript/native` (TS 7 native, the compiler). The `typescript` devDependency is pinned to **6.x on purpose** — it is the JS compiler API used by typescript-eslint, type-coverage and `scripts/generate-api-types.ts`. TS 7 ships no JS API, so bumping it breaks `check` (Dependabot/Renovate are configured to skip `typescript` >=7) |
