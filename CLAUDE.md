...
- **API versioning**: date-based. Header `Appstrate-Version` — request override, echoed on every response. Resolution order: header > org pinning (`settings.api_version`) > `CURRENT_API_VERSION`. No deprecation signalling exists — nothing emits a `Sunset` header. `middleware/api-version.ts`.
- **Idempotency**: `Idempotency-Key` is honoured on exactly the routes that mount `idempotency()` (`middleware/idempotency.ts`) — Redis-backed, 24h TTL, SHA-256 body hash, `409` concurrent, `422` body mismatch, `Idempotent-Replayed: true` on cached replay. Everywhere else the header is **refused, not ignored**: `idempotencyGuard` (`middleware/idempotency-guard.ts`, mounted globally before the routers) answers `400 idempotency_not_supported` on any unsafe method whose matched route has no `idempotency()` mount; safe methods (GET/HEAD/OPTIONS) ignore it, unmatched paths still 404. The mount is the single source of truth — the guard reads it from Hono's match result, and the OpenAPI `$ref: IdempotencyKey` on each supported operation is checked against it by `test/integration/middleware/idempotency-contract.test.ts`. Adding an idempotent route means mounting `idempotency()` **and** declaring the parameter in that operation's OpenAPI path file (a module declares it in its own `openApiPaths()`).
