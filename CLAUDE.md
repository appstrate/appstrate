...
| ------------------------------------- | -------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MODULES`                             | No       | `"oidc,webhooks,core-providers"`              | Comma-separated module specifiers to load at boot. Defaults: `oidc` + `webhooks` + `core-providers` (openai/anthropic/openai-compatible API keys). Append `codex` (ChatGPT OAuth) or `claude-code` (Claude Pro/Max/Team OAuth) to enable their providers — both opt-in due to upstream ToS posture. Or any external npm specifier for operator-installed providers. Removing a module is the only way to disable its model providers — there is no finer-grained per-providerId knob. |
