...
`PI_IMAGE` and `SIDECAR_IMAGE` are a **version contract**, not two independent knobs: the agent runtime and the sidecar speak a wire protocol that changes in the same commit, and a mismatched pair boots fine then fails runs with an opaque upstream error (#1195). Two guards, both in `services/orchestrator/runtime-image-pair.ts`: the boot **fails** when the two refs are pinned to different tags, and after the pre-pull the platform **warns** when the two images on the host carry different `org.opencontainers.image.revision` stamps. `bun run docker:build:runtime` (alias `build-runtime`) builds BOTH images with the same stamp — there is deliberately no command that rebuilds one half.
