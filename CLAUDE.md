...
**Module lifecycle:** core migrations → auto-discover built-ins → registry (`MODULES` env var) → dynamic import → topological sort by `manifest.dependencies` → `init(ctx)` (runs module migrations + workers) → `createRouter()` → running → `shutdown()` (reverse order). All declared modules are required — any import or init failure is fatal. RBAC is not part of the lifecycle: permissions are declared statically in core and consumed by modules through the typed `requirePermission` helper.
...
**Disabling a module = zero footprint:** remove it from `MODULES` and it is neither imported nor initialized. No tables, no routes, no middleware, no hook handlers, no feature flags. Core knows nothing about the module's existence. Default is empty (OSS mode).
