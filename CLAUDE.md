...
`PI_IMAGE` and `SIDECAR_IMAGE` are a **version contract**, not two independent knobs: the agent runtime and the sidecar speak a wire protocol that changes in the same commit, and a mismatched pair boots fine then fails runs with an opaque upstream error (#1195). Two guards. The env schema (`@appstrate/env`, via `findImageTagMismatch` from `@appstrate/core/image-ref`) **fails boot** when the two refs carry different tags — a cross-field rule like the S3/APP_URL ones, not conditioned on `RUN_ADAPTER`. After the pre-pull, `services/orchestrator/runtime-image-pair.ts` **warns** when the two images on the host carry different `org.opencontainers.image.revision` stamps (same tag, two builds — `:latest` rebuilt on one side only). `bun run docker:build:runtime` (alias `build-runtime`) builds BOTH images with the same stamp — there is deliberately no command that rebuilds one half.
