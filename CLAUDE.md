...
| ------------------------------------- | -------- | --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MODEL_PROVIDERS_DISABLED`            | No       | `""`                                          | Comma-separated `providerId` allowlist (codex, openai, anthropic, openai-compatible) hidden from the in-app picker. Soft-disable: existing credentials keep working (token resolver, refresh worker, llm-proxy stay unfiltered); only NEW creation is blocked (403 on POST `/api/model-provider-credentials` + POST `/api/model-providers-oauth/import`). Unknown ids fail-fast at boot. Granular within a loaded module; for full removal use `MODULES`. |
| `MODULES`                             | No       | `"oidc,webhooks,core-providers"`              | Comma-separated module specifiers to load at boot. Defaults: `oidc` + `webhooks` + `core-providers` (openai/anthropic/openai-compatible API keys). Append `codex` to enable Codex OAuth (opt-in), or any external npm specifier for operator-installed providers.                                                                                                                                                                                         |
