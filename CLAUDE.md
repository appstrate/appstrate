...
**Permissions & API key scopes:** RBAC is a typed contract co-owned by core and modules. The static catalog (`AppstrateCoreResources` + `CORE_RESOURCE_NAMES`) lives in `@appstrate/core/permissions` so external modules can build typed middleware against it. The role-grant matrix and core API-key allowlist live in `apps/api/src/lib/permissions.ts` — coupled to the auth pipeline, not shippable from npm. **Built-in modules** (resources baked into core: `webhooks`, `oauth-clients`) extend `AppstrateCoreResources` directly and are gated by the platform's unified `requirePermission(resource, action)` middleware (typed against the core+module union). **External modules** extend the catalog at runtime via `AppstrateModule.permissionsContribution()` (paired with `declare module "@appstrate/core/permissions" { interface AppstrateModuleResources { … } }` for compile-time narrowing). The platform aggregates contributions at boot and merges them into `resolvePermissions(role)`, `getApiKeyAllowedScopes()` (when `apiKeyGrantable: true`), and `getModuleEndUserAllowedScopes()` (when `endUserGrantable: true`, allowing the resource on end-user OIDC tokens). Modules gate their routes with `requireModulePermission` (typed against `AppstrateModuleResources`) and `requireCorePermission` (typed against `AppstrateCoreResources`) — both exported from `@appstrate/core/permissions`. See `apps/api/src/modules/README.md#permissions` for the full pattern.
