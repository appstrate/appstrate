...
Most-touched optional vars: `MODULES` (default `oidc,webhooks,mcp,core-providers,@appstrate/module-chat` — subscription modules `@appstrate/module-codex` + `@appstrate/module-claude-code` are opt-in), `DATABASE_URL`, `REDIS_URL`, `S3_BUCKET`, `RUN_ADAPTER` (default `process`; `docker` for containers), `APP_URL`, `TRUSTED_ORIGINS`, `TRUST_PROXY`. See `docs/ENV.md` for all 121 documented vars with defaults and full notes — 99 of them the `@appstrate/env` Zod schema's complete key set, the other 22 read straight from `process.env` by modules, the sidecar or the agent container. Below them, **two** separate tables list RETIRED names that boot now refuses (4 rows for the `DOCUMENT_*` → `FILE_*` rename, 3 for the pre-1.0 renames), and one further retirement — `OAUTH_ALLOWED_INTERNAL_IDP_HOSTS` → `EGRESS_ALLOW_INTERNAL_HOSTS` — is documented in prose instead of a table, so `RETIRED_ENV_RENAMES` (`packages/env/src/index.ts`) holds 8 entries against those 7 table rows. None of the 8 is a documented var and none is in the 121. `bun run verify:env-docs` (in `bun run check`) holds the table complete against both the schema and `.env.example`, so these counts move with the code rather than with someone remembering to edit this line — read the gate's success line, not this sentence, for today's figures.
