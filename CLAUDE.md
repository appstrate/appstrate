...
Most-touched optional vars: `MODULES` (default `oidc,webhooks,core-providers,@appstrate/module-codex,@appstrate/module-claude-code`), `DATABASE_URL`, `REDIS_URL`, `S3_BUCKET`, `RUN_ADAPTER` (default `process`; `docker` for containers), `APP_URL`, `TRUSTED_ORIGINS`, `TRUST_PROXY`. See `docs/ENV.md` for all ~75 vars with defaults and full notes.
