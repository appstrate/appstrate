...
- **Feature gating**: `useAppConfig()` hook reads `window.__APP_CONFIG__` (injected into HTML at serve time via `<script>` tag, computed once at boot by `buildAppConfig()`). Returns `{ features: { billing, webhooks, googleAuth, githubAuth, smtp } }`. No API call — falls back to OSS defaults (all `false`) if undefined. Used to conditionally render routes, nav items, dashboard widgets, and onboarding steps. Module-owned features (webhooks) default to `false` and are enabled when their module is loaded. Sidebar, routes, and tabs are fully gated — disabled modules have zero UI footprint.
