...
- **Skills**: YAML frontmatter in `SKILL.md`, parsed with the **`yaml` library at the same major the skill runtime uses** (`@earendil-works/pi-coding-agent`'s `dist/utils/frontmatter.js`) so the gate and the consumer never disagree. `name` AND `description` are both REQUIRED on every path that WRITES skill content — editor create, draft save, publish, version restore, fork, ZIP/GitHub import, bundle/MCP import (ROOT package only, never a carried dependency copy) — enforced by `checkSkillMarkdown` (`@appstrate/afps-shared/companion-files`): `name` is the bare [Agent Skills](https://agentskills.io/specification) slug (1-64 code points of lowercase `a-z`/`0-9`/`-`, no leading, trailing or doubled hyphen), written INLINE on one line, and a DIFFERENT namespace from the `@scope/name` package id (`SLUG_PATTERN`, unbounded, `--` allowed); `description` non-empty, ≤1024 code points. An unreadable block (invalid YAML, non-mapping, duplicate key, non-string field, or a leading BOM — the runtime reads no frontmatter behind one and drops the skill) is `SKILL_INVALID_FRONTMATTER`. Write paths that start from BYTES must decode via `decodeSkillMarkdown` (`ignoreBOM: true`), or a default `TextDecoder` eats the BOM and hides it from the gate. Parity is exact for PARSING only: Pi merely warns on spec violations and counts UTF-16 units, so the platform is deliberately stricter — never looser, and `packages/runner-pi/test/skill-frontmatter-parity.test.ts` runs the real Pi loader to keep it that way. AFPS §3.3 says SHOULD; the platform produces these artifacts and enforces MUST. READING is deliberately lenient — `checkCompanionFiles` (the loader, `extractRootFromAfps` → run launcher) still asks only for an inline `name` via a FROZEN substring probe, because published bundles are immutable and a run must not fail on a skill nobody can fix; the gate is therefore required to accept a SUBSET of what the loader accepts. Container path `.pi/skills/{id}/SKILL.md`.
