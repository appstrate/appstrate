...
| ------------------------------------- | -------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MODEL_PROVIDERS_DISABLED`            | No       | `""`                                          | Comma-separated `providerId` allowlist (codex, claude-code, openai, anthropic, openai-compatible) hidden from the in-app picker. Soft-disable: existing credentials keep working (token resolver, refresh worker, llm-proxy stay unfiltered); only NEW creation is blocked (403 on POST `/api/model-provider-credentials` + POST `/api/model-providers-oauth/import`). Unknown ids fail-fast at boot. The shipped `.env.example` defaults to `codex,claude-code` pending OAuth model providers' activation review (PR #397). |
| `MODULES`                             | No       | `"oidc,webhooks"`                             | Comma-separated module specifiers to load at boot. Defaults to the built-in OSS modules; append external npm specifiers to extend.                                                                                                                                                                                                                                                                                                                                                                                           |
