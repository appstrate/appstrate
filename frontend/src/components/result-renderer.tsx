...
function buildResultHtml(data: Record<string, unknown>): string {
  ...
  const handledKeys = new Set([
    "summary", "tickets_created", "informational", "results",
    "emails_processed", "emails_scanned", "newsletters_found",
    "ignored_count", "tokensUsed", "state",
    "meetings_found", "meetings_prepped", "meetings_skipped",
  ]);
...
export function ResultRenderer({ data }: ResultRendererProps) {
  ...
  return (
    <div className="result-section" dangerouslySetInnerHTML={{ __html: html }} />
  );
}
