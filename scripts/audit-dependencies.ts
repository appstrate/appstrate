 ...
 *
 * Six `protobufjs` advisories are the same dependency edge reported per
 * affected range; writing the edge out six times would make a change to it six
 * edits, five of which get missed.
 */
const REASONS = {
  /**
 ...
 *
 * Re-measured 2026-09-21 against `main`: `better-auth` 1124302 (account
 * takeover on magic-link / email-OTP sign-in), which the first version of this
 * gate left out on purpose, is gone — `better-auth` is at 1.7.3. The twelve
 * `fast-uri` entries went when the hoisted copy moved 3.1.0 → 3.1.8 in
 * `bun.lock` (a single-line edit: fast-uri has no dependencies, and
...
const ACCEPTED_ADVISORIES: AcceptedAdvisory[] = [
  ...
  // ── js-yaml — only 4.1.1 is in range; 4.3.2 and 5.2.2 are not ──
  {
    id: 1123911,
    packageName: "js-yaml",
    reason:
      "Dev tree only — js-yaml@4.1.1, pinned EXACTLY by @redocly/openapi-core@1.34.15 ← " +
      "openapi-typescript, and shared with cosmiconfig ← @commitlint/cli. Parses this repo's " +
      "own OpenAPI spec and commitlint config at build time; CPU DoS only.",
    expires: "2026-12-31",
  },
  {
    id: 1138115,
