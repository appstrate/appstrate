...
-- the release that narrows the `org_role` type — the migration that recreates it
-- without `viewer`, tracked as
-- https://github.com/appstrate/appstrate/issues/1275. That migration cannot
-- cast a row still holding the value, and it is written to refuse the deploy
-- and name this file rather than fail on the cast.
--
-- ═══ WHY `0008` LEFT THESE ═══
--
...
-- record of what was once offered, and once the type is narrowed `viewer` is
-- not a value it can hold. `guest` is the successor `0008` chose for exactly
...
-- pending rows it means `0008` has not run, and the narrowing migration will
-- say so too.
--
...
--
-- `pending_after` must ALSO print 0 before the type can be narrowed, and this
