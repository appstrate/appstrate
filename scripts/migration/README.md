...
stored `'default'` sentinel rows to NULL, keeping a row whose `identity_claims`
carry the value `"default"` (a real account of that name). Run it right after
the deploy. Details in the file header.
...
## Detail — Integration JSONPaths the release refuses (script `0024`)

**Not a runbook, and it writes nothing.** `integrationManifestSchema` now parses
every manifest JSONPath (`identity_claims`, `connect.login` `jsonpath` selectors
and success criteria) with the strict subset of
`@appstrate/afps-shared/jsonpath`, and that schema runs on every READ of a
stored manifest. Forms the previous release evaluated — `$.x-auth-token`,
`$.data.0`, `$.data[00]`, a bare `sub` — make every connect and run of that
integration fail with `invalid_manifest` after the deploy. `0024` is the
pre-flight: `DATABASE_URL=… bun scripts/migration/0024-verify-integration-jsonpaths.ts`
parses every integration draft and published version with the real schema and
prints each refused path with its rewrite. Run it BEFORE the deploy; it exits
non-zero while any remains. A draft is fixed by editing it; a published version
cannot be rewritten (integrity hash), so publish a fixed version and move its
users onto it. Details in the file header.
...
| 0022 | not applied         | `integration_connections.account_id` = `'default'` (the old no-identity sentinel) → NULL, except rows whose `identity_claims` hold `"default"` — **run right after deploying the release carrying drizzle `0071`**                                                                                                                                                                                                                                                                                                                                                                                                        | unmeasured — prints before/after counts, aborts if any row still reads `'default'`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 0023 | not applied         | unrevoked `api_keys` whose `key_prefix` starts with `ask_` (the retired format, refused with 401 `api_key_format_retired`) → `revoked_at` = now() — **run once after deploying the release carrying the `apst_` format**                                                                                                                                                                                                                                                                                                                                                                                                  | unmeasured — prints before/after counts, aborts if any `ask_` key is left unrevoked                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 0024 | not applied         | READ-ONLY pre-flight: integration drafts and published versions whose JSONPaths (`identity_claims`, `connect.login` `jsonpath` selectors and criteria) the strict subset refuses on read, each printed with its rewrite (`$.x-auth-token` → `$['x-auth-token']`, `$.data.0` → `$.data[0]`, `sub` → `$.sub`) — **run BEFORE deploying; exits non-zero until every one is fixed**                                                                                                                                                                                                                                           |
