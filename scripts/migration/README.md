...
## Detail — Generation-settings key casing (script `0024`)

**Not a runbook.** `ModelGenerationSettings` spells its reasoning effort
`reasoning_level`; `0024` renames the stored `reasoningLevel` keys in the four
...
## Detail — Identity-claim key casing (script `0025`)

**Not a runbook.** Identity-claim keys are snake_case (`account_id`,
`avatar_url`, `team_name`, …) and `extractIdentity` keys the account on
`account_id` only. `0025` renames every camelCase top-level key of
`integration_connections.identity_claims` generically. Run it once, after the
deploy. Details in the file header.

## Detail — Notification payload key casing (script `0026`)

**Not a runbook.** `notifications.payload` is returned verbatim by
`GET /api/notifications`, and `packageId` / `runId` are universal names
(CASING_CONVENTIONS 4b). `0026` renames the stored `package_id` / `run_id`
keys. Run it inside the deploy window — each build reads only its own
spelling. Details in the file header.

## Detail — Space branding key casing (script `0027`)
...
`accent_color`, `support_email`, `from_name`). `0027` renames the stored
...
## Detail — Platform run-log key casing (script `0028`)

**Not a runbook.** `run_logs.data` is returned verbatim on the run-log route and
SSE; the platform writes its keys snake_case (`integration_id` on
`integration_dropped`, `exit_code` on `firecracker_console`). `0028` renames the
stored `integrationId` / `exitCode` keys. Nothing reads the key back, so run it
once, after the deploy. Details in the file header.
...
| 0024 | not applied         | `reasoningLevel` → `reasoning_level` inside `runs.generation_config`, `runs.generation_config_override`, `space_packages.generation_config` and `package_schedules.generation_config_override` — **run inside the deploy window**                                                                                                                                                                                                                                                                                                                                                                                         | unmeasured — prints before/after counts, aborts if any row still carries `reasoningLevel`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 0025 | not applied         | camelCase → snake_case for every top-level key of `integration_connections.identity_claims` (`accountId` → `account_id`, `avatarUrl` → `avatar_url`, …; generic regexp, an existing snake_case twin wins) — **run once after the deploy**                                                                                                                                                                                                                                                                                                                                                                                 | unmeasured — prints the rows to rewrite (and those holding both spellings) before, aborts if any camelCase key survives                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 0026 | not applied         | `package_id` → `packageId`, `run_id` → `runId` inside `notifications.payload` — **run inside the deploy window**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | unmeasured — prints before/after counts, aborts if any row still carries `package_id` or `run_id`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 0027 | not applied         | `logoUrl` / `primaryColor` / `accentColor` / `supportEmail` / `fromName` → snake_case inside `spaces.settings.branding` (an existing snake_case twin wins) — **run inside the deploy window**                                                                                                                                                                                                                                                                                                                                                                                                                             | unmeasured — prints before/after counts, aborts if any camelCase branding key survives                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 0028 | not applied         | `integrationId` → `integration_id` (`integration_dropped` rows) and `exitCode` → `exit_code` (`firecracker_console` rows) inside `run_logs.data` — **run once after the deploy**                                                                                                                                                                                                                                                                                                                                                                                                                                          | unmeasured — one sequential scan of `run_logs`; prints before/after counts, aborts if any camelCase platform key survives                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
