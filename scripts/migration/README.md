...
## Detail — Identity-claim keys the release no longer reads (script `0028`)

**Not a runbook, and it writes nothing.** `extractIdentity` keys a connection
on the `account_id` identity claim only. `0028` checks every org integration
