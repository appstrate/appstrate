...
| 0024 | not applied         | READ-ONLY pre-flight for #1458: integration drafts and published versions whose templated `authorized_uris` the manifest schema now refuses on read, and `@appstrate/ssh` connections whose `host`/`port` render no egress under 1.0.1 (expected 0: the provisioner always writes `port`) — **run BEFORE deploying, with the platform env loaded; exits non-zero until every one is fixed**                                                                                                                                                                                                                               |
