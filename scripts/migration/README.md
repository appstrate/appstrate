...
| 0018 | not run yet         | TWO moments, one file. **Section A — before the drizzle batch, platform stopped, only when the pre-flight counts a row**: mint `Connexion N` for every `integration_connections` row whose `label` is NULL, ranked by `created_at` within its (space, integration, owner) group, so drizzle `0069`'s `SET NOT NULL` cannot raise 23502 and roll the batch back — the `0009`/`0013` shape. **Section B — after the batch has applied at boot**: the three connection jsonb columns rewritten from one pick per integration to a SET, a scalar or object becoming a single-element array                                    | unmeasured — each section prints its counts before and after, all of which must read 0 after. **Not run.** Idempotent both halves: `WHERE label IS NULL` and `EXISTS (… jsonb_typeof(v) <> 'array')` are each exactly the condition they remove, so a second pass matches zero rows, and an empty map (`{}`) is left alone rather than aggregated to NULL. Do NOT feed the whole file to psql in one go — section B writes a shape only the new image reads. Pre-flight both counts read-only before the window; the file ends with those queries plus a control counting every row that carries a value, so "nothing to rewrite" and "nothing at all" read differently                                      |
