...
| 0018 | not run yet         | the three connection jsonb columns — `runs.connection_overrides`, `runs.resolved_connections`, `package_schedules.connection_overrides` — rewritten from one pick per integration to a SET: a scalar or object value becomes a single-element array, an array is left untouched; **run BEFORE the drizzle batch, with the platform STOPPED** — stop, run this, deploy the new image (`0069` applies at boot), reopen. It depends on nothing `0069` does, and the new readers raise on the old shape rather than degrade, so rewriting with the platform down is what keeps any request from meeting an unrewritten row. Schedule job data in Redis needs no rewrite: at boot the scheduler upserts every enabled schedule's BullMQ job from its (rewritten) row before its worker starts, replacing the pending job, and every fire validates the `connection_overrides` it carries — a job the sync skipped still holding the old shape records a visible failed run instead of launching | **not yet rehearsed** — production holds rows in all three columns, so rehearse on a restored dump and record the counts in the file header before the window; prints the three non-array row counts before and after, all three after must be 0. **Not run.** Idempotent: the `EXISTS (… jsonb_typeof(v) <> 'array')` guard is exactly the condition each `UPDATE` removes, so a second pass matches zero rows, and an empty map (`{}`) is left alone rather than aggregated to NULL. `0069` carries the `integration_connections.label` backfill its own `SET NOT NULL` preconditions — doctrine §2, not this file. Pre-flight the counts read-only before the window; the file ends with that query plus a control counting every row that carries a value, so "nothing to rewrite" and "nothing at all" read differently |
