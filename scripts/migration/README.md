...
| 0009 | not applied | `org_invitations`: cancel older duplicate pending rows per (org, email) so drizzle `0057` can create `uq_org_invitations_pending` — **only if `0057` fails**; the old service serialised creates by cancelling, so duplicates need a racing pair                 | unmeasured — prints the duplicate-pair count before/after, after must be 0                                                             |
