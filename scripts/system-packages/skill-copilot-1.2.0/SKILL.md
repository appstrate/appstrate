...
disponibles et les méthodes de l'organisation ou de référence présentes dans ton contexte.
...
Les descriptions des skills présentes dans ton contexte sont le catalogue courant des méthodes. Une
skill que l'organisation possède l'emporte sur une méthode système de référence.
...
Pour un agent enregistré, identifie la meilleure candidate parmi les skills de l'organisation et les
méthodes système de référence. Ne crée, ne matérialise et ne modifie encore aucun package :
`@appstrate/agent-authoring` possède cette résolution et appellera `@appstrate/skill-authoring` si la
méthode manque ou doit être améliorée.
