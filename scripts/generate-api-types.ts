...
/**
 * Banner written at the top of the output: how to regenerate, and where the
 * merged spec comes from — `collectModuleOpenApi` reaches `packages/module-*`
 * too, `packages/module-ee` included, so a source-available module's `Ee*`
 * schemas and `/api/billing/*` paths land in this Apache-2.0 file, and removing
 * that directory is how a redistributor drops them.
 *
 * `AUTO-GENERATED by <script>` is load-bearing: it is the marker
 * `verify-license-boundary.ts` reads to exempt this file — the ONE file in the
 * repository with no SPDX header — from the licence gate.
 */
