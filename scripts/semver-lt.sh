...
# Mixed dotted/undotted prerelease identifiers (`ab` vs `a.b`) follow dpkg, not
# SemVer; real tags (`beta.N`, `rc.N`) are unaffected.
#
...
# dot-separated identifiers then compare as SemVer does (beta.9 < beta.10 < rc.1).
deb_version() { sed 's/^v//; s/-/~/' <<<"$1"; }

# 0/1 are dpkg's verdict (odd characters only draw a warning); anything else is
# a dpkg failure (crash, invalid invocation) and must not read as "not lower".
rc=0
