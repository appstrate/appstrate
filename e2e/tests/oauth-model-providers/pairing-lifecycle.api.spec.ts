 ...
 * live server with a real registered provider (`codex` from the default
 * `@appstrate/module-codex` module set). It exercises the full operator
 * loop observable from the dashboard:
 *
 *   1. POST /pairing            (mint, cookie-auth)
 *   2. GET  /pairing/:id        → pending
 *   3. POST /import bad bearer  → 410          (replay-proof)
 *   4. POST /import (helper sim, bearer-auth, body-supplied accountId/email
 *      bypasses the JWT identity hook — exact path used by
...
test.describe("OAuth Model Providers — pairing lifecycle @smoke", () => {
  test("mint → consume → list → revoke (full cross-link contract)", async ({
    ...
    //    Body carries `accountId` + `email`, which take precedence over
    //    the codex module's JWT identity hook (the helper does this when
    //    the OAuth response body already surfaces the identity slots).
    const SYNTHETIC_ACCOUNT_ID = "00000000-1111-4222-8333-444444444444";
    const SYNTHETIC_EMAIL = `pairing-e2e-${Date.now()}@example.test`;
    ...
    expect(imported.providerId).toBe(PROVIDER_ID);
