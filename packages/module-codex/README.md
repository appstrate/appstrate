...
> **Execution status.** Codex agent runs are **executable**. They run on the single Pi engine (`@earendil-works/pi-coding-agent`) like any other subscription provider — there is no separate "official binary" path and no per-provider execution-engine binding. Pi's SDK (`@earendil-works/pi-ai`) natively emits the codex-responses OAuth request shape (`chatgpt-account-id`, the codex user-agent), so the platform forges nothing: the real subscription token never enters the agent container (it gets a placeholder bearer), and the sidecar `/llm` oauth branch swaps in the real token server-side (bearer-swap only). Subscription runs still require an isolating orchestrator (docker/firecracker): the `process` adapter runs the agent and its sidecar as host processes of one user, so nothing keeps the agent from the sidecar's environment. This remains an operator opt-in grey-zone choice — see [`docs/architecture/SUBSCRIPTION_COMPLIANCE.md`](../../docs/architecture/SUBSCRIPTION_COMPLIANCE.md). Append `@appstrate/module-codex` to `MODULES` to enable it.
>
> **Chat:** Codex is **chat-usable.** `openai-codex-responses` is in `CHAT_USABLE_FAMILIES`, so a connected Codex subscription appears in the chat model picker and is driven by the same generic in-process Pi chat engine as claude-code (`@appstrate/module-chat`, `src/pi-chat/`).
