...
import { buildCodexPlaceholderIdToken } from "@appstrate/core/jwt";
 ...
 * shape-based heuristics for a subprocess's stderr) and {@link redactSecretsDeep}
...
export function buildCodexAuthJson(opts: {
  ...
  // Single source of truth for the synthetic id_token shape (shared with the
  // API-side module-codex provider) — alg:none, local-only, never transmitted.
  const idToken = buildCodexPlaceholderIdToken(accountId, opts.nowMs);
  return {
    auth_mode: "chatgpt",
    tokens: {
...
export function buildCodexConfigToml(opts: {
  /**
   * The platform meta-tools server — REQUIRED. A Codex vend run ALWAYS needs the
   * sidecar (for `GET /credential-vend`), so the `[mcp_servers.platform]` entry
   * is always emitted; there is no no-MCP Codex mode on the real launch path.
   */
