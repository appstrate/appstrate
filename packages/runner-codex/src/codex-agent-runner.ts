...
export interface CodexAgentRunnerOptions {
   ...
   * tools the Claude runner gets through the Agent SDK's `mcpServers`. REQUIRED:
   * a Codex vend run ALWAYS needs the sidecar (for `GET /credential-vend`), so a
   * no-sidecar Codex run cannot occur on the real launch path.
   */
...
export class CodexAgentRunner implements Runner {
  ...
  private async prepareCodexHome(cred: VendedCodexCredential, nowMs: number): Promise<string> {
    ...
    await writeCodexConfig({
      home,
      toml: buildCodexConfigToml({ platform: this.opts.sidecarMcp }),
    });
    return home;
  }
