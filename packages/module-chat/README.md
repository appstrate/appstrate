...
- **Identité** : forward des headers de l'appelant (cookie/Authorization + X-Org-Id/X-Space-Id) sur appels loopback — l'OAuth audience-bindé du satellite disparaît, le pipeline d'auth ré-authentifie chaque saut. Les bearers frappés par le module (`src/loopback-auth.ts`, secret process-local, un tour) déclarent `principalKind: "user"` : le loopback EST l'appelant par un autre transport, donc espaces personnels et grants par principal répondent au tour du moteur exactement comme à sa requête `/api/chat` (cf. `docs/architecture/RBAC_PERMISSIONS_SPEC.md` §7.4).
