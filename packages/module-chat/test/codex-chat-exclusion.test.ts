...
describe("codex chat exclusion", () => {
  it("CHAT_USABLE_FAMILIES does not include the codex apiShape", () => {
    expect(CHAT_USABLE_FAMILIES.has(CODEX_API_SHAPE)).toBe(false);
  });

  it("proxyTarget returns null for the codex family (no route)", () => {
    expect(proxyTarget(CODEX_API_SHAPE)).toBeNull();
  });

  it("pickModel never selects a codex model, even when it is the default", () => {
    const chosen = pickModel([{ ...codex, isDefault: true }, openai]);
    expect(chosen.id).toBe("m_openai");
  });

  it("pickModel throws a clear error when only codex is connected", () => {
    expect(() => pickModel([codex])).toThrow(/Aucun modèle utilisable/);
  });

  it("explicitly requesting a codex preset id throws a clear error", () => {
