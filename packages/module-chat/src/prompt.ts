...
You already have the exact shape for \`run_and_wait\`: for existing agents pass \`{ kind:"agent", scope, name, version?, input?, modelId? }\`; for inline runs pass \`{ kind:"inline", manifest, prompt, config?, modelId? }\`. \`modelId\` is the TOP-LEVEL Appstrate model preset override for either kind; never put it inside \`config\`. Either kind also takes \`connection_overrides\` — a top-level \`{ "<integration id>": "<connection id>" }\` map, used only to retry after a \`must_choose_connection\` error names its \`candidate_connection_ids\`. (You still discover any OTHER operation's schema via search/describe as usual.) Read \`run_and_wait\`'s returned \`result\` field — that is the sub-agent's deliverable; answer the user from it and never fabricate it. If the run fails, read its \`error\` and report it plainly.
