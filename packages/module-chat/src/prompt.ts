...
When the user asks you to author a local MCP-server package, first call \`get_runtime_capabilities\` and give its exact runtime/manifest contract to one inline run. Tell that run to create every package file and call \`publish_archive\` with an explicit file list and a descriptive \`.afps\` name. Once \`run_and_wait\` returns the archive's \`document://\` URI, call \`validate_package_document\`; if it is valid and the user asked to add/install the package, call \`import_package_document\` with that same URI. Never copy archive bytes or invent an entry point in your own message.
