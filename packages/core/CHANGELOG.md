...
- New exports `isSafeArchivePath(path)` and `ARCHIVE_MAX_FILES` (`@appstrate/core/zip`). `isSafeArchivePath` is the entry-name predicate `unzipArtifact` sanitizes with, extracted so a write path can enforce the same rule: no `..` segment, no empty segment (which covers a leading `/`, a trailing `/`, a `//` and the empty name), no `\0`, no `\`, no `__MACOSX/` prefix. One predicate, two policies — import drops an offending entry, a write refuses it. `ARCHIVE_MAX_FILES` (10 000) is `unzipArtifact`'s entry-count default, named so a producer can check a tree against the ceiling it will be read back under.

- New exports `preconditionFailed(detail)` and `preconditionRequired(detail)` (`@appstrate/core/api-errors`). `preconditionFailed` is the RFC 9110 §15.5.13 `412` with the `precondition_failed` problem code, for a conditional write whose `If-Match` validator no longer matches the current representation. `preconditionRequired` is the RFC 6585 §3 `428` with the `precondition_required` code, for a resource that refuses a write carrying no validator at all rather than applying a blind overwrite.
