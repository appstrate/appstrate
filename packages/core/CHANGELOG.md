...
- `@appstrate/core/chat-turn-metadata` — the chat turn's TIME budget alongside its
  ...
  `CHAT_MIN_RUN_BUDGET_MS`, `CHAT_THIN_RUN_BUDGET_MS`, plus the pure
...
- `@appstrate/core/run-and-wait-client` — `RUN_RESULT_INLINE_MAX_BYTES` (32 KB),
  `RUN_RESULT_SPILL_DOCUMENT_NAME`, `runResultExceedsInlineLimit()` and
  `truncateRunAndWaitPayload()`: a run result over the cap is cut to a usable head
  plus a `document://` pointer at the full copy. `launchRunAndWait` forwards the
  new `context_documents` argument (inline runs only).
