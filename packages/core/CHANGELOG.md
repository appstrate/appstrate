...
- New export `preconditionFailed(detail)` (`@appstrate/core/api-errors`): RFC 9110 §15.5.13 `412` with the `precondition_failed` problem code, for a conditional write whose `If-Match` validator no longer matches the current representation.
