 ...
 *
 * Without a single source of truth the two paths drift: a `.afps-bundle` can
 * pass the runtime loader and fail at platform import (or vice-versa). This
 * helper centralizes the §3.3 / §3.4 companion-file invariants so both
 * call sites reject the same inputs.
 *
 * Implementation note — `@appstrate/afps-runtime` intentionally carries no
 * runtime dependency on `@appstrate/core` (see `archive-utils.ts`). The
 * runtime mirrors this helper in `packages/afps-runtime/src/bundle/companion-files.ts`
 * with the same algorithm; the two implementations MUST stay in lockstep.
 * When changing the rules here, change them there too — and update the
 * conformance test that asserts parity.
 */
