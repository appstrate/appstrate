...
/**
 * Index the agent's selected {@link RuntimeToolDef}s by their bare descriptor
 * name (`"log"`, `"output"`, …). The map IS the first-party allowlist: a tool
 * name that isn't a key is not a runtime tool (integration tools are namespaced
 * `{ns}__{tool}` and never match), so callers gate replay on `map.has(name)`.
 */
export function buildRuntimeToolDefMap(
  opts: BuildRuntimeToolDefsOptions,
): Map<string, RuntimeToolDef> {
  const map = new Map<string, RuntimeToolDef>();
  for (const def of buildRuntimeToolDefs(opts)) map.set(def.descriptor.name, def);
  return map;
}

/**
 * Reconstruct a runtime tool's canonical events by replaying its PURE handler
 * on the observed call args — the transport-agnostic capture every runner
 * shares. Used when the transport dropped the result `_meta` (the Claude SDK +
 * the Codex CLI both do) or to avoid coupling capture to `_meta` at all: the
 * handler only packages args into events (the reducer applies the effect once
 * at finalize), so replaying it locally yields the same events the sidecar's
 * copy produced. Returns the allowlisted canonical events WITHOUT `runId` — the
 * caller stamps `runId` when emitting on the run's sink. A handler error (e.g.
 * `output` schema validation failed) yields `[]` (the model already saw the
 * error text from the tool's own result).
 */
export async function replayRuntimeToolEvents(
  def: RuntimeToolDef,
  args: unknown,
  ...
  try {
    ...
    // The handler rejected (e.g. `output` args violate the schema). The model
    // already saw the tool's own error result; there is no canonical event to
    // reconstruct.
    return [];
  }
  const events: RuntimeToolEvent[] = [];
  reEmitRuntimeToolEvents(result._meta, (ev) => events.push(ev));
  return events;
}
