...
- `./vite` export — Vite preset that pre-bundles the schema-form CJS dep graph (RJSF, ajv/dist/2020.js, ajv-formats, fast-uri, jsonpointer, react-select, lucide-react). Consumers merge it via `mergeConfig(appstrateUi, ...)` instead of maintaining the include list themselves.
