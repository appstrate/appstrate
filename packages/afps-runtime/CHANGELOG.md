...
- **`canonicalBundleDigest(bundle: Bundle)`** — single signature, takes a `Bundle` directly. Internally extracts the root package's non-reserved files (excludes `RECORD` + `signature.sig`) and produces the same canonical `JSON([ [sortedPath, "sha256-<b64>"], … ])` digest. Callers no longer maintain their own root-files flatteners.
