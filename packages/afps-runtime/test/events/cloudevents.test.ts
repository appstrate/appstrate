...
describe("buildCloudEventEnvelope", () => {
  ...
  it("never stamps dataschema — not even for a well-formed canonical event", () => {
    // The attribute used to point at `schemas.afps.dev/v0/events/*`. Nothing
    // was served there, and the AFPS RunEvent payload is deliberately open —
    // the spec reserves namespaces, not shapes — so the URIs asserted a
    // normative form AFPS never adopted. Emitting it again is a spec change
    // first (see the module header). This asserts absence for EVERY canonical
    // type, so reintroducing the stamp for one of them cannot slip through.
    for (const type of CANONICAL_EVENT_TYPES) {
      ...
      expect({ type, hasAttribute: "dataschema" in env }).toEqual({ type, hasAttribute: false });
    }
  });

  it("builds canonical and third-party envelopes identically apart from type", () => {
    // With the stamp gone, canonicality has no effect on the envelope at all.
    ...
    expect(canonical).toEqual({ ...thirdParty, type: "log.written" });
  });
