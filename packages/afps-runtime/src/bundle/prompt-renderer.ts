...
/**
 * Resolve the final agent prompt by assembling a {@link PromptView}
 * from the {@link ExecutionContext} and rendering the template with
 * logic-less Mustache. The output depends only on the template + view,
 * never on hidden I/O during rendering.
 */
...
function sliceMemories(
  memories: MemorySnapshot[] | undefined,
  limit: number,
): PromptView["memories"] {
...
function sliceHistory(history: HistoryEntry[] | undefined, limit: number): PromptView["history"] {
