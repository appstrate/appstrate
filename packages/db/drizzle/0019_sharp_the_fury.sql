...
CREATE UNIQUE INDEX "uq_runs_id_org_id" ON "runs" USING btree ("id","org_id");--> statement-breakpoint
...
-- in a maintenance window (VALIDATE takes only a SHARE UPDATE EXCLUSIVE
-- lock, so it is safe to run online).
