...
-- row's rank by `created_at` over the WHOLE (space, integration, owner) group,
-- labelled rows included — the service's own COUNT(*)+1 rule. Ranking over the
-- NULL subset alone would mint a second "Connexion 1" beside an existing one.
...
UPDATE "integration_connections" c SET "label" = 'Connexion ' || r.rank FROM (SELECT "id", row_number() OVER (PARTITION BY "space_id", "integration_package_id", coalesce("user_id", "end_user_id") ORDER BY "created_at", "id") AS rank FROM "integration_connections") r WHERE c."id" = r."id" AND c."label" IS NULL;--> statement-breakpoint
