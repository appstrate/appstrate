...
-- space of the org. The single `space_id` FK becomes a composite
-- `(space_id, org_id)` FK so a space row cannot name another org's space.
--
...
ALTER TABLE "integration_oauth_clients" DROP CONSTRAINT "integration_oauth_clients_space_id_spaces_id_fk";--> statement-breakpoint
...
ALTER TABLE "integration_oauth_clients" ADD CONSTRAINT "integration_oauth_clients_space_id_org_id_fk" FOREIGN KEY ("space_id","org_id") REFERENCES "public"."spaces"("id","org_id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
