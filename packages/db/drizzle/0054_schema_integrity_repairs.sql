...
-- invitation with it).
--
-- `chat_messages.format` / `parent_id` — the two the table's own doc already
-- described as carrying no information, deferred to "a coordinated change with
-- `packages/module-chat`". This is that change; the module's DTO and its
-- OpenAPI response schema drop both in the same commit. `format` had exactly
-- one possible value in every row that ever existed (`persistence.ts` wrote a
-- constant at both the insert and the conflict-update). `parent_id` was a
-- re-encoding of `seq` order under a strictly linear writer, unconstrained by
-- any FK or uniqueness, and no reader ever reconstructed a tree from it — the
-- client's history load reads `id` and `content` and nothing else.
--
-- THE PARENT MESSAGE ID ITSELF IS NOT DEAD and is deliberately untouched: it is
-- one third of the material `deterministicMessageId` hashes for a message that
-- arrives without an id. It stays computed from `lastMessageId` and threaded
-- into that hash exactly as before, so every `gen_…` id already persisted still
-- reproduces. Only the STORED column goes.
--
-- `IF EXISTS` on all four, same convergence reasoning as the `IF NOT EXISTS`
...
ALTER TABLE "chat_messages" DROP COLUMN IF EXISTS "format";--> statement-breakpoint
