...
-- `api_keys`, … — each under the lock the deletion holds. The action this
-- migration enables is exactly the thing that would be slow without them, so
-- they belong in the same file.
--
...
-- either way (0029's header makes the same argument). The scan cannot fail: it
-- re-validates the constraint that is already there, with a weaker action.
--
