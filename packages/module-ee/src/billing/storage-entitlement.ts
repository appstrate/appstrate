 ...
 * fallback.
 *
 * It was `setDocumentStorageLimit` until the platform's #1177 rename, and this
 * binding is why that spelling outlived every other alias: the name is read off
 * the LIVE services object the platform injects, not off this module's pinned
 * `PlatformServices` type, so a platform-side rename does not rename the read
 * here — it turns the next boot into a `TypeError`. Which means the two must
 * move in lockstep: this line ships BEFORE (or with) the platform build that
 * drops the old name.
 */
