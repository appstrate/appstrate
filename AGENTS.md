...
- **Extension failure = throw**: Pi ignores an `isError` on a returned result — only a throw from `execute` becomes a tool error (log `Tool error`, provider `is_error`). Return through `toPiToolResult` (`@appstrate/runner-pi`), which throws an `isError` result.
