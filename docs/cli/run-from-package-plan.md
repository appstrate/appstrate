...
1. Endpoint `GET /api/applications/{applicationId}/packages/{scope}/{name}/run-config` qui retourne le payload résolu :
   ```json
   {
     "config": { ... },
...
**Tâches** (côté CLI) 2. Avant `runCommandInner` : si profil + applicationId disponibles, `GET …/run-config`. Merger dans cet ordre (priorité décroissante) :
...
3. Persister le `connectionProfileId` choisi dans la config CLI (à côté de `applicationId`/`orgId`). Lecture par `runCommand` comme valeur par défaut, override par flag.
