...
- Standalone single-binary builds for `linux-x64`, `linux-arm64`, `darwin-x64`, `darwin-arm64`, and `win32-x64`, produced by `bun build --compile --target=<target>` and attached to GitHub Releases.
- A refactored `https://get.appstrate.dev` shell one-liner that detects OS/arch, downloads the matching binary to `/usr/local/bin/appstrate`, and chains into `appstrate install`.
