...
| Named wire carve-outs (4c, 4e, 4n; asymmetries 5c, 5d) — by literal name only            | **camelCase**                                 | `displayName`, `providerId`, `hasMore`, `modelId` on `/api/models*` |
...
**Exception within Zone 1**: the carve-out names of Zone 4 and the asymmetries of Zone 5 stay camelCase on the wire, each by its literal name.
...
| `POST /api/model-provider-credentials/discover` body           | `providerId` (4e)                                                                                                                | `credential_id`, `api_key`, `base_url_override`                                                                                                                    |
...
| `POST /api/model-providers-oauth/pairing` body                 | `providerId`                                                                                                                     | `credential_id`                                                                                                                                                    |
| Pairing mint response                                          | `id`, `expiresAt`                                                                                                                | `token`, `command`                                                                                                                                                 |
| `GET /api/model-providers-oauth/pairing/{id}`                  | `id`, `expiresAt`                                                                                                                | `status`, `consumed_at`, `credential_id`                                                                                                                           |
...
| `POST /api/model-providers-oauth/pair/redeem` 200              | `providerId`                                                                                                                     | `credential_id`, `email`, `available_model_ids`                                                                                                                    |
...
| `notifications.payload`                                                                                                                         | `agent_id`, `status` (`run_completed`); `packageId`, `package_type`, `shared_by_name` (`package_shared`)                                 | `GET /api/notifications`                                               |
...
| `integration_connections.identity_claims`                                                                                                       | the manifest's `identity_claims` keys — snake_case, enforced by core validation (`packages/core/src/integration.ts`)                     | connection DTOs                                                        |
...
**Why**: an LLM-facing tool schema, not a REST body — agent prompts and skills are written against these names, so renaming one silently breaks every agent that uses it. The platform's own MCP server (`apps/api/src/modules/mcp/tools.ts`) is a different surface and follows Zone 1: `list_files` takes `run_id`, `chat_session_id`, `purpose`; `invoke_operation` takes `if_match`.
...
#### 5c — Model/proxy/credential ids on the standalone endpoints

**Standalone endpoints** use **camelCase** wire fields `modelId`, `proxyId`, `credentialId`:

- `GET`/`PATCH /api/agents/{scope}/{name}/model` (`modelId`) and `GET`/`PUT /api/agents/{scope}/{name}/proxy` (`proxyId`)
- the org model surface `/api/models*` (`modelId`, `credentialId`; `PUT /api/models/default` takes `modelId`), org-scoped by the `X-Org-Id` header rather than by a path segment
...
**Every other surface uses snake_case**: the run-launch bodies (`POST /api/agents/{scope}/{name}/run`, `POST /api/runs/inline`, the MCP `run_and_wait` tool) and the chat body (`POST /api/chat`) take `model_id` / `proxy_id` (chat: `model_id`); schedules take `model_id_override`, `proxy_id_override`, `version_override`; `/api/models/test` takes `existing_model_id` next to its 5c ids.

End-to-end consistent within each surface (backend Zod ↔ OpenAPI ↔ frontend hook all match). These ids appear on too few types to qualify for Carve-out 4b. If a new model/proxy/credential endpoint is needed, it uses snake_case wire.
...
The actual OAuth 2.0 wire endpoints (`/oauth2/authorize`, `/oauth2/token`, the CLI token grant's `client_id`) stay snake_case per RFC 6749 — only the management surface is camelCase. Treat any new management route over a BA plugin table the same way; for non-plugin tables the default snake_case wire rule applies.
...
- `auths.{key}.identity_claims.{claim}` — claim keys are snake_case (`account_id`, `avatar_url`, `team_name`, …), and so are `connect.login.identity_outputs` entries; core validation refuses any other key
...
**Run-launch and chat bodies** (snake_case): `input`, `rerun_from`, `model_id`, `proxy_id`, `generation`, `connection_overrides`, `dependency_overrides` (agent run); `manifest`, `prompt` on top for an inline run; chat: `messages`, `model_id`, `generation`, `agent_authoring`. The remote-run body keeps the 4b `contextSnapshot` beside `sink.ttl_seconds`.
...
**Model-provider family** (4e): the per-object table under Carve-out 4e — `api_key`, `base_url`, `base_url_override`, `existing_key_id`, `provider_name`, `available_model_ids`, `candidate_count`, `credential_id`, `model_ids`, `promoted_default`, `existing_model_id`, `oauth_email`, `consumed_at`, `access_token`, `refresh_token`, `account_id`.
...
| Gate                                                                                           | What it holds                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/api/test/unit/openapi-casing-carve-out.test.ts`                                          | Walks the built OpenAPI spec (core + every module) and DISCOVERS names: every property name, query parameter name and example/default key holding an uppercase letter must be in its exported `CAMEL_CASE_CARVE_OUTS` (name → section of this document); every entry must still be in use. It also fails on the snake_case twin of a 4b or pagination name outside `SNAKE_TWIN_EXCEPTIONS` (exact JSON pointers). Example subtrees that are opaque data (`input`, `checkpoint`, webhook `payload`, `headers`) are not walked. |
...
| Core integration validation (`packages/core/src/integration.ts`)                               | `identity_claims` keys and `connect.login.identity_outputs` entries are snake_case.                                                                                                                                                                                                                                                                                                                                                                                                                                           |
