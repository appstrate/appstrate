...
| JSONB `runs.metadata` (platform-written, returned verbatim on the run DTO) | **snake_case**                                | `{ degraded_integrations }` (wire payload)                  |
...
**Why**: TS function-argument convention. Hook params are TS interfaces, not wire DTOs. Fields like `BeforeUsageParams.runningCount` or `RunStatusChangeParams.modelSource` are camelCase — TS contracts, not JSON wire fields.
...
**Library DTO domain fields** (snake_case — `LibraryPackageList` and its `PackagePlacement` entries on `GET /api/library` and `GET /api/spaces/{id}/library`): `home_space_id`, `home_writable`, `home_shareable`, and on each placement `space_id`, `via`, `state`, `shared_by.{user_id, name}`. `space_id` and `user_id` are the two look-alikes of the universal DB carve-out below and they stay **snake_case here**, deliberately: they are this DTO's own domain fields, sitting beside `home_space_id` in the same object and spelled exactly as `PackageShare.shared_by.user_id` already spells them. Two ids of the same kind disagreeing inside one object would cost more than either convention buys, so the carve-out records the exception instead of the audit re-finding it.
...
13. **Is it a key inside `runs.metadata` JSONB?** → snake_case (the column is returned verbatim on the run DTO, so its interior is wire).
