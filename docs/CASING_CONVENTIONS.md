...
| Named wire carve-outs (4c, 4e, 4n; asymmetries 5c, 5d) — by literal name (4n, 5d: per surface) | **camelCase**                                 | `displayName`, `providerId`, `hasMore`, `modelId`           |
...
**Rule — name-based**: these names stay camelCase **wherever they appear** in the model-provider family (credentials, org models, OAuth pairing, provider registry): `providerId`, `apiShape`, `authMode`, `displayName`, `iconUrl`, `defaultBaseUrl`, `baseUrlOverridable`, `featured`, `contextWindow`, `maxTokens`, `capabilities`, `cost` (with `cost.{input, output, cacheRead, cacheWrite, tiers}`, each tier the same rates plus `inputTokensAbove` — Pi's registry shape), `docsUrl`. Every OTHER field of these objects is Zone 1 (`api_key`, `base_url`, `base_url_override`, `provider_name`, `available_model_ids`, `access_token`, `refresh_token`, `account_id`, `consumed_at`, `model_ids`, `promoted_default`, `live_model_search`, `pi_provider`, …), except the 4b names and the 5c ids (`modelId`, `credentialId`). A nested object that is not a 4e name is Zone 1 all the way down: `generation.reasoning.temperature_compatible`.
...
| `POST /api/model-provider-credentials/test` body          | `providerId` (4e), `credentialId` (5c)                                                                                                              | `base_url`, `api_key`                                                                                                                                                             |
| `POST /api/model-provider-credentials/discover` body      | `providerId` (4e), `credentialId` (5c)                                                                                                              | `api_key`, `base_url_override`                                                                                                                                                    |
| `ModelProviderCredential`                                 | `id`, `apiShape`, `authMode`, `providerId`, `createdAt`, `updatedAt`                                                                                | `label`, `base_url`, `source`, `oauth_email`, `needs_reconnection`, `created_by`                                                                                                  |
| `OrgModel`                                                | `id`, `apiShape`, `providerId`, `modelId` (upstream id, 5c), `credentialId` (5c), `contextWindow`, `maxTokens`, `iconUrl`, `createdAt`, `updatedAt` | `label`, `provider_name`, `base_url`, `generation`, `input`, `reasoning`, `cost`, `enabled`, `is_default`, `needs_reconnection`, `aliased`, `source`, `created_by`, `pi_provider` |
| `POST /api/models/seed` body                              | `credentialId` (5c)                                                                                                                                 | `model_ids`                                                                                                                                                                       |
| `POST /api/models/seed` 201                               | —                                                                                                                                                   | `created`, `ids`, `promoted_default`                                                                                                                                              |
| `POST /api/models/test` body                              | `credentialId`, `modelId` (upstream id) (5c)                                                                                                        | `api_key`                                                                                                                                                                         |
...
**The one exception — `run_logs.data`** (`GET /api/runs/{id}/logs`, `run_log` SSE, returned verbatim) is the event payload in its producer's vocabulary: runtime/sidecar CloudEvents camelCase (4i, e.g. `integrationId`, `durationMs`, `toolCallId`); platform-written markers reuse that vocabulary (`integrationId` on `integration_dropped`, `exitCode` on `firecracker_console`); the platform's `file` row is `{file_id, uri, name, mime, size, sha256}`.
...
**Rule**: the configuration the platform serializes into a sidecar's or agent container's environment is camelCase (`PI_LLM_OAUTH_CONFIG_JSON`, `PI_MODEL_SWAP_JSON`: `authMode`, `clientApiShape`, `backingApiShape`, `backing.{providerId, reasoning, input, …}`; `MODEL_COST`: `ModelCost`).
...
- the org model surface `/api/models*`: `credentialId`; `PUT /api/models/default` takes `modelId`
...
**Other names are Zone 1**: a schedule's overrides are `model_id_override`, `proxy_id_override`, `version_override`. The MCP `run_and_wait` tool takes no model or proxy.
...
- **Route references ≥2 packages → `{packageId}`** (one param holding `@scope/name`). Route pattern `/:packageId{@[^/]+/[^/]+}`. Used by `/api/integrations/*` (runtime tier), where routes like `/integrations/{packageId}/agent-resolution/{agentPackageId}` carry two packages in one path — two `{scope}/{name}` pairs would be ambiguous to parse. One explicit exception: `/api/org-integrations/*` references a single package yet uses `{packageId}`, because a tier twin keeps its sibling's `{packageId}` shape so both share parameters and schemas.
...
**Run log data** (4g): producer vocabulary — markers `integrationId`, `exitCode` (4i); the `file` row `file_id`, `uri`, `name`, `mime`, `size`, `sha256`.
...
- `integrationId` → **`integration_id`** (a `run_logs.data` event payload keeps its producer's `integrationId`, 4g); `homeSpaceId` → **`home_space_id`**.
...
| Gate                                                                                           | What it holds                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `apps/api/test/unit/openapi-casing-carve-out.test.ts`                                          | Walks the built OpenAPI spec (core + every module) and DISCOVERS names: every property name, query parameter name and example/default key holding an uppercase letter must be in its exported `CAMEL_CASE_CARVE_OUTS` (name → section of this document). An entry is name-based (valid anywhere) except the 4n surface names and the 5d names, which carry the JSON-pointer prefixes of their surface and fail elsewhere — 4n: the API-key, proxy, space / end-user (with the library's space rows) and webhook components and paths, the list-envelope names staying valid anywhere; 5d: the `OAuthClient*` components and `/api/oauth/clients*`, `/api/auth/cli/*` and `/api/orgs/{orgId}/cli-sessions*`. Every entry must still be in use within its scope. It also fails on the snake_case twin of a 4b or pagination name outside `SNAKE_TWIN_EXCEPTIONS` (exact JSON pointers). Example subtrees that are opaque data are not walked: `input`, `checkpoint`, `headers` anywhere, `payload` only under `/api/webhooks*` (a notification `payload` is walked). |
