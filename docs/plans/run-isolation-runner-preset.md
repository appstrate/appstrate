...
| File                                                                                            | Change                                                                                                                                                                                                                                                                                                                                                                                    |
| ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/core/src/permissions.ts:80`                                                           | `runs: "read" \| "read-all" \| "cancel" \| "delete"` + doc comment: `read` = runs the principal launched; `read-all` = every run in the space (other members, end-users, legacy actor-less rows), and `read-all` IMPLIES `read` — the guard on every run read surface is the disjunction `requireRunsRead`, so a role granted only the wider permission is not a role that reads nothing. |
...
| `POST /api/runs/:id/cancel`                                | `routes/runs.ts:606`                                                                                 | `assertRunVisible` after the row load. The `cancel` permission stays as it is — it gates the _action_, visibility gates _which rows_. There is no `DELETE /api/runs/:id`: a single run is never deleted on its own.                                                                                                                                                                                                                                       |
| `DELETE /api/agents/:scope/:name/runs` (bulk)              | `routes/runs.ts:821`                                                                                 | The one run mutation with no row to check — it spans every run of the agent in the space, so it takes `runs:read-all` alongside `runs:delete`. Without it a principal scoped `runs:delete` alone would erase what it cannot see.                                                                                                                                                                                                                          |
...
| `packages/db/drizzle/0060_runner_preset.sql`     | Three CHECKs, not two: `spaces_default_role_valid` and `space_members_preset_valid` WIDEN to admit `'runner'` (`0056_space_roles.sql:70,143`), and `space_roles_key_not_preset` NARROWS to reserve the key — so the file first counts the custom roles already keyed `runner` and RAISEs, naming the rename, because the bare constraint violation names the constraint and not the remedy. Snapshot + `_journal.json` entry (`idx: 60`, `when` > 0059's — see the drizzle index-collision memo: build the snapshot by hand and diff it against 0059's). |
...
| Route guards in `app.tsx`                                           | the agent and skill EDITOR routes keep their `:write` guard rather than dropping to `:read`. `:write` is strictly tighter, it already excludes a runner (which holds no `:write` either), and `PackageEditorPage` has no save gating of its own — so `:read` would open an editable form to a caller the server refuses on save. |
...
- Migration **0060** (three CHECK constraints — two widened, one narrowed).
  Auto-applied at boot; no operator step, no env var. It refuses to run when an
  organization already defined a custom role keyed `runner`, and names the
  rename in the message.
...
1. **PR A** — `runs:read-all` + predicates + SSE + files arm + tests + CHANGELOG
   ...
   release note.
2. **PR B** — `runner` preset + migration 0060 + summary read + SPA gating +
   tests + CHANGELOG _Added_. Depends on A for "a runner sees only their runs".
3. Production deploys the whole RBAC lot at once: 0056 → 0060 in one boot
