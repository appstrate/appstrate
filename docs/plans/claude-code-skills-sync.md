...
the selected spaces in their pinned Appstrate organization is
...
skill is unchanged. Skill sources are selected by repeated `--space` flags, then
profile `syncSpaces`, then the pinned space; packages are deduplicated by ID.
