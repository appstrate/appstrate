...
| D17 | Sync warns when the pinned space is not in `GET /api/spaces`, and syncs the reachable spaces anyway                                                                                                                      | Failing the run; falling `.mcp.json` back to another space      | D14 removed the pin from skill selection, so a dead pin no longer surfaces as a failed listing — but D12 still sends it as `X-Space-Id`, and the server refuses a header naming a space the principal cannot reach. Skills would be current while MCP silently 403s. The run has the space list in hand, so the note costs nothing; changing the MCP space on the user's behalf would move their execution context without asking |
...
skill is unchanged. Skill sources default to every space this profile reaches;
repeated `--space` flags, then profile `syncSpaces`, narrow that set. The
