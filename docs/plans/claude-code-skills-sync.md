...
| D9  | One installation per destination = the currently synchronized profile context, which is `profileName` + `instance` + `userId` + `orgId` — four fields, the pinned space NOT among them                                                                 | One plugin per profile / org; counting the pinned space as part of that identity | The marketplace entry is a static string; multi-org users switch with `appstrate org switch` or run `--profile` from cron. D14 took the pin out of skill selection, so `appstrate space switch` installs the very same skills: the pin is `.mcp.json` content (D12), which is compared BY CONTENT and rewritten when it differs. Counting it as identity bought nothing and cost the all-or-nothing rule — one package that persistently fails to resolve blocked a header rewrite. A pin that moves DURING a run still aborts the swap, because the `.mcp.json` bytes were computed from it before the downloads started                                                                 |
...
plugin's skills and generated MCP connection are prepared and replaced together;
a failed replacement preserves the previous plugin. The MCP header always uses
the pinned active space, even when skills are selected from several spaces —
and the pin is not part of the context (D9): switching space rewrites
`.mcp.json` and keeps every installed skill, without the all-or-nothing rule a
context switch carries. A pin that moves mid-run still aborts the swap.
...
skills only; they do not introduce package authoring or bidirectional folders.
