...
**Zero callable tools is a broken agent, refused at both ends**: a _declared_ integration whose **effective** selection is empty (`tools: []`, or `tools` absent on an integration that declares no `default_tools`) exposes nothing callable. That is a contract violation, not a degraded mode — the agent's prompt is written against the declared surface, so leaving it declared-but-empty makes the model improvise unauthenticated HTTP instead of stopping. Two gates, one rule:
...
- **Wherever a version is frozen** (`requireCallableTools` in `apps/api/src/services/integration-scope-validation.ts`) — cutting a version, ZIP/GitHub import, `.afps-bundle` import, and the initial snapshot the create routes take (`createVersionSafe`, `routes/packages.ts`) refuse the manifest with a `no_tools_selected` field error on `integrations_configuration.{id}.tools`, naming the two ways out: select a tool, or drop the `dependencies.integrations` entry. This is where the author can still fix it. The create routes themselves keep ACCEPTING the empty manifest — the editor's flow passes through it — they simply do not freeze a version from it.
  ...
  **Not a complete barrier, by construction.** The validator reads integrations from the DB, so an integration carried BY a `.afps-bundle` and not yet installed is invisible to it and its agent is not judged on that entry — the same documented "not installed → skip silently" short-circuit `/import` has. Say "every path that freezes a version, for every integration already known to the org", not "every import"; the runtime backstop is what makes the remaining case safe rather than silent.
...
The publish gate is **not** covering a blind spot of the boot gate — an earlier write-up claimed it was, by inverting the spawn resolver's logic, and that claim is wrong. An integration declaring **no** `auths` is not invisible: `resolveDeliveries` (`integration-spawn-resolver.ts:655-658`) returns `{ spawnEnv: {} }` for an empty `auths` map, so `resolveOne` continues and a spec **is** emitted; the sidecar sees the integration and `assertIntegrationExposesTools` does fire. The spec is suppressed (`resolveDeliveries` → `null`, `:672-679`) only when the integration declares an auth and no accessible connection resolves — and readiness already refuses that run with `not_connected`, exactly as it refuses an uninstalled integration with `integration_not_active` (`agent-readiness.ts`). So every "no spec emitted" path is already gated, just by a different, more precise error.
...
The gate reads the **pinned** integration manifest, not the integration's live draft: it resolves each `dependencies.integrations.{id}` range through `resolveRunIntegrationVersions` — the same resolver that freezes `runs.resolved_integration_versions` — so "empty" at publish means what it will mean at boot. An agent pinned to `^1.0.0` whose v1 declares `default_tools: ["api_call"]` publishes fine even after the integration author drops `default_tools` from their draft. A pin that resolves to nothing (never published / unsatisfiable) leaves the entry **unjudged**: that run fails loud upstream with `dependency_unresolved` (422), and a gate that blocks a legitimate publish would be worse than one that defers to the backstop. The `tools`/`scopes` **subset** checks still read the draft — a pre-existing drift, unchanged here.
...
- **Phase 1 (install-time, `apps/api/src/services/integration-scope-validation.ts`)** — `POST/PUT /api/packages/agents` + ZIP import refuse manifests whose `integrations_configuration[id].tools[]` selection includes an undeclared tool, or whose `scopes[]` selection escapes the `auths.{key}.scope_catalog` (per target auth). Field paths surface as `integrations_configuration.{id}.tools` / `integrations_configuration.{id}.scopes`. Configuration without a matching dep is rejected at manifest validation (the dep table is the canonical "is this integration declared" gate). The `requireCallableTools` option adds the declared-but-empty gate on top, and is opt-in per call site: **on** for publishing a version, for `POST /api/packages/import`, and for `POST /api/packages/import-bundle`, **off** for the draft writes (see "Zero callable tools" above). On `/import-bundle` the gate runs as a pure-read preflight in `handleImportBundle` (`services/bundle-import.ts`), before conflict detection and before the first write, so one invalid agent aborts the **whole** bundle with no rollback; each field error's message is prefixed with the offending `{packageId}@{version}` identity. It is one call into the shared validator, so the `tools[]` / `scopes[]` subset checks — previously missing on this route entirely — come along with it: one gate, identical semantics, on every path that freezes an artifact. Residual: an integration carried BY the bundle and not yet in the org registry is invisible to the validator (it reads the DB), so a self-contained foreign bundle is not judged on that integration — the same documented "not installed → skip silently" short-circuit `/import` has, with readiness and the boot gate still behind it.
