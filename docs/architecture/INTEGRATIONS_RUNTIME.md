...
### AFPS Integrations — per-URL TLS-client routing (`_meta["dev.appstrate/tls-client"]`, #403)

Some upstreams (behind Cloudflare / Akamai Bot Manager, or with JA3/JA4 TLS fingerprinting) reject the TLS ClientHello emitted by Bun/undici `fetch` because it doesn't match a real browser. The `_meta["dev.appstrate/tls-client"]` vendor extension declares, per integration, which upstream URLs to route through `curl` (its own TLS stack) — or, for SOTA browser mimicry, `curl-impersonate` (replicates Chrome/Firefox cipher suites, extension order, GREASE, HTTP/2 settings). Everything not matched keeps the default Bun-`fetch` path.

- **Manifest shape**: `{ routes: [{ url_pattern, client, impersonate? }] }`. `url_pattern` reuses the `authorized_uris` glob format. `client` is `"undici"` (the default, explicit so a narrow route can override a broader one) or `"curl"`. `impersonate` (curl only) names a curl-impersonate target (`"chrome"`, `"firefox"`, …); absent → plain curl. **First matching route wins.** Validated by the install-time superRefine in `@appstrate/core/integration` (`getTlsClientRoutes` + `TLS_CLIENT_META_KEY`).
- **Wiring**: the spawn resolver copies the resolved routes onto `IntegrationSpawnSpec.tlsClientRoutes` (dropped for remote HTTP MCP — those requests don't traverse the MITM listener). At boot, `integrations-boot.ts` wraps the MITM listener's upstream `fetch` with `makeTlsClientFetch(routes)` (`tls-client-router.ts`). The wrapper is a drop-in `fetch`: it terminates **nothing** of the existing path — credential injection, SSRF re-check, 401 retry, and response passthrough are all unchanged; only which client makes the listener↔upstream TLS handshake differs. No routes → the default `fetch` is returned unchanged (zero overhead).
- **curl runner** (`curl-runner.ts`): spawns `curl` via `Bun.spawn` per request, `-q` (ignore curlrc), `-D -` (headers→stdout), `--compressed` (match Bun fetch auto-decompress), `--max-time` / `--max-filesize` from the sidecar limits, manual redirect (no `-L`), body via stdin + `--data-binary @-`. Binaries are overridable via `TLS_CLIENT_CURL_BIN` (default `curl`) and `TLS_CLIENT_CURL_IMPERSONATE_BIN` (default `curl-impersonate`). The sidecar image ships `curl`; curl-impersonate is opt-in (vendor a build, point the env var at it).
- **Scope**: v1 covers the MITM listener path (local integrations using `delivery.http` to a TLS-exotic provider). The `api_call` proxy and the plain CONNECT egress listener still use the default client — both inject a `fetch` the same way, so extending routing to them is mechanical follow-up (not yet wired).
- **Operator observability**: routed requests emit a `tls-client route` info log `{ integrationId, url (query stripped), client, impersonate }`.
