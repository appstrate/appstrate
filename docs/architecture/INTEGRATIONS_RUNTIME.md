...
**Zero callable tools fails the run**: a _declared_ integration that ends boot with 0 registered tools is a boot-contract violation, not a degraded mode — the sidecar records it in the boot report's `failed[]` and the agent container aborts the run (`assertIntegrationExposesTools` in `runtime-pi/sidecar/integrations-boot.ts`). The agent's prompt is written against the declared surface; leaving it declared-but-empty makes the model improvise unauthenticated HTTP instead of stopping. Deselecting every tool therefore means removing the `dependencies.integrations` entry too.
