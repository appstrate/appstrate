...
| `PATCH`  | `/api/spaces/{id}/packages/{scope}/{name}`            | per package TYPE            | CONFIGURE only: `modelId` / `proxyId` / `generation_config`, all three under `configure`, which the personal-space waiver never covers. The body is `.strict()` and carries no `enabled` — activation has its own pair of doors                                                                                                                |
