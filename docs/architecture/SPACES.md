...
| `POST`   | `/api/spaces/{id}/packages`                           | per package TYPE            | `agents:configure` / `skills:write` / `mcp-servers:write` / `integrations:install` — waived in the caller's own personal space. A package not yet placed here also needs `<type>:share` in its home; the share is then created with the install |
| `GET`    | `/api/spaces/{id}/packages/{scope}/{name}`            | `spaces:read`               |                                                                                                                                                                                                                                                 |
| `PUT`    | `/api/spaces/{id}/packages/{scope}/{name}`            | per package TYPE            | `modelId` / `proxyId` / `generationConfig` need `configure`; `enabled` is a switch of presence and is judged as install/uninstall — the `POST`/`DELETE` strings, waived in the caller's own personal space. A body doing both clears both gates |
