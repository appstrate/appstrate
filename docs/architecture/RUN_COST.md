...
1. **Platform-side runs on an org credential (Pi container)**: `org_models.cost` (or the catalog) → `ResolvedModel.cost` → `PromptContext.llmConfig.cost` → snapshotted at kickoff onto `runs.model_cost` → the runner's cumulative token counters arrive on the `appstrate.metric` event → **one cumulative `source="runner"` row per run** (`writeRunnerLedgerRow`), priced **server-side** from `runs.model_cost` and those counters (see _Pricing_ below). The container computes a cost of its own too (`MODEL_COST` env var → Pi SDK `calculateCost` → `RunMessage.cost`), but the platform does NOT record it: the number billing debits credits from must not be produced inside the sandbox the platform is isolating, and the platform already holds both factors. Computing it server-side is also what lets `MODEL_COST` be withheld from a container running an aliased model, whose published rate card would otherwise identify the vendor the alias exists to hide. The reported figure survives as a cutover instrument only: at the terminal ledger barrier, once per run, a divergence beyond 1e-6 USD logs `llm_usage: runner-reported cost diverges from the server-computed cost`. DB models (`org_models`) also support an optional `cost` (jsonb); OpenRouter models auto-populate it from the pricing API. The dashboard exposes no pricing fields (operator-only, by design); `PATCH /api/models/{id}` with `{"cost": {...}}` sets the override and `{"cost": null}` clears it back to the catalog.
2. **Server-side proxy (`/api/llm-proxy/*`, remote runs + chat's API-key turns; `/internal/llm-proxy/*`, platform runs on a platform-provided model)**: resolves the preset, forwards upstream, and inserts **one `source="proxy"` row per call**, priced from `ResolvedModel.cost` (see _Pricing_ below). Pricing comes from `ModelDefinition.cost`, identical to the platform chain. A platform run whose model is a `SYSTEM_PROVIDER_KEYS` preset (`isMeteredByPlatformProxy`) is served by the second entry: its sidecar holds no vendor key and calls the proxy with the run token, the proxy serves the run's own model (`runs.model_id`, never the request's `model`), and each call lands as a proxy row on the run. Such a run writes **no** runner row (`writeRunnerLedgerRow` returns early on the same predicate), so its spend is the proxy rows alone. The deploy window is the exception: a system run launched before migration `0072` has no pinned model (`runs.model_id`), holds its own credential and keeps its runner row.
...
4. **In-process chat engine** (`chat-platform-services.ts:recordChatUsage`): the Pi chat engine drives every chat turn, but only its oauth-subscription branch (claude-code, codex) talks to the provider directly; that branch never traverses the proxy, so it meters its own turn — one row per turn, attributed by `chat_session_id`, always `credential_source="org"` (it spends the user's own subscription), priced like the runner row (see _Pricing_ below). It is stamped `source="proxy"`, which is a **known labelling inaccuracy**: the enum has only `proxy | runner`, and `proxy` is the correct half of that pair here because the row is immutable at insert (see _Settlement_). A dedicated third enum value would need a DB enum migration plus a `PlatformServices` contract change.
...
## Pricing — Pi's `calculateCost`, tiers per request only

Rate cards come from Pi's model registry (pinned `@earendil-works/pi-ai`), OpenRouter's price list for OpenRouter models, or an operator override on `org_models.cost`. Every row is priced by Pi's own `calculateCost` (`piTokenCostUsd` in `packages/runner-pi/src/pi-model.ts`, wrapped for the ledger in `apps/api/src/services/token-cost.ts`), the function the container also prices with. A rate card may carry **price tiers** (`cost.tiers`, e.g. `inputTokensAbove: 272000` on OpenAI's GPT-5.x): the highest threshold that ONE request's input + cache-read + cache-write exceeds reprices that whole request.

- **Proxy row** — one upstream request: priced exactly, tiers honoured (`requestCostUsd`).
- **Runner row, subscription chat turn** — usage summed over several requests, which no longer says how large any one was: priced at the **base rate**, tiers stripped (`aggregatedCostUsd`). This under-prices a run or turn in which some request crossed a threshold; a per-request `max_request_input_tokens` counter would let these rows detect it.
- **Container's reported figure** (a cross-check, never billed) — `runtime-pi/env.ts` drops the tiers from `MODEL_COST`, so its per-request sum equals the server's base-rate row and a run past a threshold raises no false divergence.

`apps/api/test/unit/runner-cost-parity.test.ts` pins the three with hand-computed prices.

## Token buckets — one normalisation, two implementations

Cost is `input×input_rate + output×output_rate + cacheRead×cacheRead_rate + cacheWrite×cacheWrite_rate` over four **disjoint** buckets, at the rates the request's tier selects. The same upstream reply is normalised into those buckets twice — by the proxy adapter (`llm-proxy/openai.ts`) and by `@earendil-works/pi-ai` (`api/openai-completions.js`, used by every platform-side run). **The two must agree, or identical consumption costs a different amount depending on where the run executed.** The openai-compatible formula (pi-ai's, mirrored exactly by the adapter):
...
`prompt_tokens` is the TOTAL prompt in every dialect (OpenAI `cached_tokens ⊂ prompt_tokens`; DeepSeek `prompt_tokens = hit + miss`), so both cache buckets are carved back out of `input`. `cached_tokens` is the cache-READ count and nothing else: OpenAI neither documents nor emits `cache_write_tokens`, and the OpenRouter-compatible providers that do emit it report it as a **separate** count rather than folding it into `cached_tokens` ([OpenRouter's own provider](https://github.com/OpenRouterTeam/ai-sdk-provider/pull/409), [ds4](https://github.com/antirez/ds4/pull/29)). Subtracting writes from `cached_tokens` therefore under-reports a spec-compliant provider, which is why pi-ai stopped doing it in 0.84 and the adapter follows. Anthropic's wire fields are already disjoint and map 1:1. Parity is enforced by `apps/api/test/unit/llm-proxy-usage-parity.test.ts`, which also fails if the installed pi-ai stops matching the transcribed formula. `apps/api/test/unit/runner-cost-parity.test.ts` also pins the precondition that keeps pi-ai's 1h-cache-write branch (2× the input rate, a term the ledger does not carry) unreachable: every `Model` the platform builds goes through `buildPiModel` (`packages/runner-pi/src/pi-model.ts` — container, sidecar, chat, CLI), which applies `PLATFORM_MODEL_COMPAT` (`compat.supportsLongCacheRetention: false`, the single flag pi-ai gates every long-retention emission on; the same constant also turns off two record fields the meter cannot price, see `model-compat.ts`). That refusal is what makes the precondition hold, not the fact that the platform never injects `PI_CACHE_RETENTION=long`: pi-ai resolves retention from `options.cacheRetention` and then from the AMBIENT `process.env`, and agent code runs inside the container, so what the platform injects is only half the story. The sidecar additionally declines to forward a container-supplied `cacheRetention` option (`FORWARDED_OPTION_KEYS`), which is a boundary decision rather than the billing guarantee.
...
The price above is permissive on purpose: it is `0` for an absent `cost`, and an absent cache rate prices at `0`. That is correct arithmetic, but it made `cost_usd = 0` unattributable — a genuinely free subscription-backed call, a model the platform failed to price, and a call whose cached fraction was priced at zero were the same row.

`llm_usage.pricing_status` records which one it is. `classifyTokenPricing` (`@appstrate/afps-runtime/runner`) is the only place the rules live:

- **`unpriced`** — no rates at all (`ResolvedModel.cost == null`): a user-described gateway model (`openai-compatible`, `anthropic-compatible`) with no operator rate card, or a Pi record that states no price — so this is reachable, not theoretical. The `0` is an absence of pricing.
- **`partial`** — the model had rates, but a bucket that carried tokens had none: `cost.cacheRead` absent while `cache_read_input_tokens > 0`. Those tokens were already carved out of `input` by the normalisation above, so they are billed in no bucket at all. The figure is a floor. A missing `cacheWrite` rate deliberately does NOT trigger this — several vendors bill no write premium, and rate cards not read from Pi's registry (an override, a gateway's price list) routinely omit it, so flagging it would mark correctly priced rows `partial`.
...
The degenerate-event skip in `writeRunnerLedgerRow` keys on the same distinction — the input the row's cost is DERIVED from. A platform run needs a usage snapshot (a cost-only event recomputes to `0` and would mint an all-zero row); a remote-origin run needs only the reported cost.
...
Usage reporting is opt-in on the openai-compatible wire, so the adapter forces it (`LlmProxyAdapter.prepareRequest` → `stream_options.include_usage`, the same hook that refuses with a 400 what the proxy cannot meter) on **every** preset it forwards, system or org-owned — billing must not depend on the caller SDK setting a flag. The decision lives on the adapter (the protocol registry), never on an `apiShape` check in the core: adding a fifth API shape means implementing the hook, not editing `core.ts`.
...
- **Terminal ledger barrier** — `finalizeRun` writes the terminal snapshot with `required: true` **before** the CAS that settles the run, on _every_ path and **regardless of `result.cost`**: platform-synthesised terminals (stall watchdog, boot orphan sweep, container crash/timeout/cancel) build an empty `RunResult` that never carries `cost`, and those are precisely the runs whose last snapshot is in doubt. `required` propagates a write failure, so the run stays open and finalize is retried rather than settling on a snapshot that is not durable. A run that consumed nothing (no tokens, no cost) has no accounting fact to pin and mints no row. The row is priced server-side from `runs.model_cost` × the preserved snapshot rather than from `result.cost`. That rarely moves the number — the upsert is monotone, so the metric side-channel's rows already hold the run's cost and a lower candidate cannot regress them. It does guarantee a priced row where the container reported no cost at all, which is every aliased run.
...
- **No durable retry for runner rows** — a failed runner write PROPAGATES instead of being enqueued (`llm-usage-retry.ts`). An asynchronous replay could only land after the run settled, where the rule above refuses it. Propagation is also the correct recovery for a cumulative producer: on the metric path it rolls back the ingestion transaction (the sequence is not advanced, the runner re-POSTs, and its next cumulative snapshot supersedes the lost one), and at finalize it keeps the run unsettled. The exception is a failure caused by the row's own values (SQLSTATE class 22 or 23514), which would fail identically on every retry and wedge the event stream (#1501): it does not roll back — `persistEventAndAdvance` claims the sequence with a `system`/`event_dropped` run_logs row instead of the event and logs one error, and the runner's next cumulative snapshot still supersedes the dropped one. The placeholder also catches a platform bug that raises one of those codes — a deliberate trade-off: the stream keeps moving and the drop is logged at error level rather than retried forever. **Proxy rows keep the queue** — each is an immutable per-call fact that receives a fresh serial id when it lands, so a late replay is still swept and billed.
...
A `source="runner"` row is written for every run that consumed tokens except a platform run on a platform-provided model (see producer 2); it is **never** conditionally skipped because proxy rows exist. Double-counting is avoided at **read** time, by the single shared predicate `notRunnerMirrorSql` (`state/runs.ts`):
...
- **Float precision**: `runs.cost`, `llm_usage.cost_usd` and `credential_proxy_usage.cost_usd` are `doublePrecision` (IEEE-754). Summing many sub-cent per-call costs accumulates rounding drift, so `runs.cost` can disagree with a re-summation by sub-cent amounts — relevant because `@appstrate/module-ee` bills off this data. Migrating to `numeric` is still deferred, but no longer for coordination reasons: `postgres.js` returns `numeric` as a **string**, so every read site (here + `packages/module-ee`) must parse it — one repo, one PR since the module came in-tree.
