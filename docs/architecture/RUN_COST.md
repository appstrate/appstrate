...
4. **In-process chat engine** (`chat-platform-services.ts:recordChatUsage`): the Pi chat engine drives every chat turn, but only its oauth-subscription branch (claude-code, codex) talks to the provider directly; that branch never traverses the proxy, so it meters its own turn — one row per turn, attributed by `chat_session_id`, always `credential_source="org"` (it spends the user's own subscription), priced by the same `computeTokenCost`. It is stamped `source="proxy"`, which is a **known labelling inaccuracy**: the enum has only `proxy | runner`, and `proxy` is the correct half of that pair here because the row is immutable at insert (see _Settlement_). A dedicated third enum value would need a DB enum migration plus a `PlatformServices` contract change.
