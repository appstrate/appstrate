...
2. **Server-side proxy (`/api/llm-proxy/*`, remote runs + chat's API-key turns; `/internal/llm-proxy/*`, platform runs on a platform-provided model)**: resolves the preset, forwards upstream, and inserts **one `source="proxy"` row per call**, priced from `ResolvedModel.cost` (see _Pricing_ below). Pricing comes from `ModelDefinition.cost`, identical to the platform chain. A platform run whose model is a `SYSTEM_PROVIDER_KEYS` preset pinned at launch (`runs.model_source = 'system'` and `runs.model_id` set — `isMeteredByPlatformProxy`) is served by the second entry: its sidecar holds no vendor key and calls the proxy with the run token, the proxy serves the run's own model (`runs.model_id`, never the request's `model`), and each call lands as a proxy row on the run. Such a run writes **no** runner row (`writeRunnerLedgerRow` returns early on the same predicate), so its spend is the proxy rows alone; a system run started before `runs.model_id` existed keeps its runner row.
