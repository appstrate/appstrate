...
- **Product consequence**: a `preview_url` opened in its own tab (shared link, "open in new tab") shows the document's **source**, not the rendered page — in every mode. The SPA's preview modal is unaffected: it loads the URL in an iframe, which still renders.
- **Separate origin (optional)**: `USERCONTENT_URL` points a second registrable domain (eTLD+1) at the same server → the browser gives the preview its own cookie jar, storage partition, and process (site isolation). When set, `preview_url` is minted on that origin. It does **not** unlock a top-level active render (above); its value is defence in depth for the render that does happen — an independent boundary that survives a reverse proxy or CDN stripping the CSP header and a UA that ignores `sandbox`, neither of which the route can detect at runtime. Cloud always sets it. Because that residual layer is unverifiable at runtime, the env schema refuses to boot when `USERCONTENT_URL` shares `APP_URL`'s **host** (identical value, or the same host on another port/scheme — cookies are host-scoped, not origin-scoped), and requires `https://` in production. A different registrable domain is stronger than a different host but is not machine-checked (correct eTLD+1 detection needs the Public Suffix List), so it stays the documented recommendation while a different host is the enforced floor.
...
- **What the sandbox does NOT buy — self-navigation.** A sandboxed navigable may **always navigate itself**; the sandboxed-top-level-navigation flags gate navigating an **ancestor** only. Two consequences:
  ...
  - **Inside the SPA's preview modal iframe** (the one context that IS served active): a plain no-target link **still navigates the frame in place** — links are not disabled there. `target="_blank"` (no `allow-popups`) and `target="_top"` (no `allow-top-navigation`) are dead, and were already dead before the CSP `sandbox` existed, because the iframe element has carried `sandbox="allow-scripts"` all along.
- **Limitation (pre-existing, not fixed here)**: from that same self-navigation freedom, an agent document rendered in the modal can replace **its own frame** with an arbitrary page — including a convincing fake login. Constraining that requires a `frame-src` directive on the **SPA's** own response, and the app currently sends no CSP at all, so there is nothing to extend. Tracked as a separate gap.
...
| `USERCONTENT_URL`                     | unset (same-origin preview) | Separate registrable domain for serving HTML previews (strongest isolation). Boot fails if it shares `APP_URL`'s host.     |
