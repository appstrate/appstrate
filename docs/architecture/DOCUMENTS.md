...
- **Separate origin (optional)**: `USERCONTENT_URL` points a second registrable domain (eTLD+1) at the same server → the browser gives the preview its own cookie jar, storage partition, and process (site isolation). When set, `preview_url` is minted on that origin and HTML is served as active content in **any** loading context — the script has no app origin to reach. Cloud always sets it. Because presence alone unlocks that unconditional-active branch, the env schema refuses to boot when `USERCONTENT_URL` shares `APP_URL`'s **host** (identical value, or the same host on another port/scheme — cookies are host-scoped, not origin-scoped), and requires `https://` in production. A different registrable domain is stronger than a different host but is not machine-checked (correct eTLD+1 detection needs the Public Suffix List), so it stays the documented recommendation while a different host is the enforced floor.
- **Same-origin mode (the OSS default, `USERCONTENT_URL` unset)**: `preview_url` is minted on `APP_URL`. The CSP blocks exfiltration (`connect-src`, `form-action`, `img-src`, `base-uri`) but it does **not** block script execution — the page has to render — so what matters is _which origin_ the script runs in. Inside the SPA's `sandbox="allow-scripts"` iframe that origin is opaque and the isolation holds. But `preview_url` is an absolute URL with a 300 s token: opened **top-level** (new tab, shared link) the sandbox attribute does not exist and agent script would run on `APP_URL` itself — reaching the SPA's `localStorage`/`sessionStorage`, non-HttpOnly cookies, and same-origin navigation. So in same-origin mode the route serves `html` as **active HTML only for a proven nested-document load** (`Sec-Fetch-Dest: iframe`, a browser-set header script cannot forge); every other context — top-level navigation, `fetch`, `object`/`embed`, and a **missing** header (non-browser client, or a browser too old to send it) — is served as inert `text/plain; charset=utf-8` source with `default-src 'none'`. Fail-closed, and the response carries `Vary: Sec-Fetch-Dest`. Degrading rather than erroring keeps a shared link readable: the token holder sees the markup as source, which it could download anyway, but nothing executes. The decision lives in one function, `mayServeActiveHtml()` (`services/document-preview.ts`).
...
- **HTML** is the only ACTIVE-content kind and keeps the full treatment: a strict `Content-Security-Policy` header **and** a parse-time `<meta http-equiv>` CSP injected as the first child of `<head>` (`injectMetaCsp`) — covers the relative-URL / `srcdoc` bypass a header alone can miss. In same-origin mode that treatment is applied only to an iframe load; any other context gets the inert `text/plain` degradation described above.
- **CSP `sandbox` (header only)**: the header copy carries `sandbox allow-scripts` and no other token, putting the response in an **opaque origin**. It is the only directive in the policy that restricts navigation of a **top-level browsing context** — without it, a preview opened top-level (new tab, shared link) could steer that tab to the real `/login` and phish credentials on a real first-party origin (GHSA-8f6g-r37m-wg99). `allow-popups*` is rejected for the same reason (a popup to `/login` reopens the chain), `allow-top-navigation*` because the attack _is_ a user-initiated click, `allow-same-origin` because it hands the origin back. The injected `<meta>` copy omits `sandbox`, which a meta context ignores; `buildPreviewCsp()` therefore returns `{ header, meta }` so the two cannot be swapped. COOP `same-origin` can never match an opaque origin, which puts the document in its own browsing-context group — strictly stronger, so it stays.
- **What the sandbox does to links, per loading context.** A sandboxed navigable may **always navigate itself**; the sandboxed-top-level-navigation flags only gate navigating a **top-level** browsing context. So the two paths differ:
  - **Top-level load** (the `USERCONTENT_URL` path — preview opened in its own tab or from a shared link): the preview document _is_ the top-level browsing context, so a plain `<a href="https://…">` click, a `location = …` assignment and a `<meta http-equiv="refresh">` are all blocked. That is the phishing fix, and the accepted cost of it.
  - **Inside the SPA's preview modal iframe**: the document is a **nested** navigable, so a plain no-target link **still navigates the frame in place** — links are not disabled there. `target="_blank"` (no `allow-popups`) and `target="_top"` (no `allow-top-navigation`) are dead, but both were already dead before the CSP `sandbox` existed, because the iframe element has carried `sandbox="allow-scripts"` all along. **The modal path is unchanged by the header sandbox.**
- **Limitation (pre-existing, not introduced or fixed here)**: because self-navigation is always permitted, an agent document rendered in the modal can replace **its own frame** with an arbitrary page — including a convincing fake login. Constraining that requires a `frame-src` directive on the **SPA's** own response, and the app currently sends no CSP at all, so there is nothing to extend. Out of scope for the preview hardening; tracked as a separate gap.
- **What a TOP-LEVEL preview load gives up to the opaque origin.** The sandbox is not free — an interactive agent report loaded top-level loses capabilities it would have on a normal origin:
  - `localStorage`, `sessionStorage` and `document.cookie` access **throws `SecurityError`** rather than returning empty. A script that touches storage **aborts at that point** — it does not degrade gracefully — so an agent report that persists UI state must guard those calls in a `try`/`catch`.
  - No downloads from inside the document (`<a download>`, blob "export" buttons) — no `allow-downloads`.
  - No `alert` / `confirm` / `prompt` — no `allow-modals`.
  - No popups, and no top-level navigation (above).
  - Framed accurately: this makes the **top-level** path behave the way the SPA modal path **already** behaved — that iframe has always been an opaque origin with the same token set. The top-level load was the odd one out, not the other way round. This is the accepted cost of closing the phishing hole, not a surprise regression.
...
| **`200`**      | The bytes. `html` → `text/html` under the strict CSP + injected meta CSP, or `text/plain` source in same-origin mode outside an iframe; `image`/`pdf` → stored mime, `inline`; `text` → always relabelled `text/plain; charset=utf-8`. Always `nosniff`, `no-referrer`, `private, no-store`, COOP `same-origin`, CORP `same-origin` (or `cross-origin` when `USERCONTENT_URL` is set) |
