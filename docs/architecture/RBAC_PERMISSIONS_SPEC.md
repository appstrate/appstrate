...
The rule has three readers in TypeScript plus one mirror in SQL, and they differ only in the set they compare against: the catalog check above, the per-space visibility gate of the package read routes (`isPackageReadableInSpace`), the library projection that serves both `GET /api/library` and `GET /api/spaces/{id}/library`, and the per-type index listing (`listOrgItems`, whose SQL mirrors the predicate — the `activeOnly` narrowing the integration picker uses stays install-only, since the home is not a usable instance). The organization-catalogue exception sitting beside it is `home_space_id IS NULL ∧ managesOrgCatalog` and nothing more: a package the organization owns stays the organization's however it is placed, so sharing one takes nothing away from the catalogue.
