...
The owner's own preset is `admin` — **except a `guest`, who holds `operator`**. A guest is an external identity with no implicit reach into any space (§3.2), invited to USE one thing; `admin` in their own space would let them author and launch arbitrary agents on the organization's LLM budget, which is precisely what their org role withholds. `operator` is receive-and-run: read, launch, manage their own connections, no `<type>:write`. That constraint is honoured by package placement (§6.10): **installing into one's OWN personal space requires none of the type's install grants** (`spacePackagePermission(type, "install")` — `agents:configure` for an agent, `integrations:install` for an integration, `<type>:write` for the rest). `POST /api/spaces/{spaceId}/packages` is the one install door, a personal space included, and `gateSpacePackageWrite` skips both the coarse and the exact grant check when the target space's `owner_user_id` is the caller (`callerPersonalOwnerId`): **ownership IS the authorization**, and an `operator` holds none of those grants — requiring them would mean a recipient could never take up what somebody offered them. Putting it back down is the same act, so `enabled` on `PUT /api/spaces/{id}/packages/{scope}/{name}` is judged as `install` / `uninstall` and is exempt with them. What is NOT skipped: the catalog read, so a package the caller cannot reach is a 404 in their own space too, and the placement itself, re-read under the share row's lock inside `installPackage`'s transaction. `configure` keeps its grant even at home — `modelId`, `proxyId` and `generationConfig` spend the organization's LLM budget, which is precisely what a guest's org role withholds. A guest therefore installs, uninstalls and enables what was offered to them, and never chooses the model it runs on.
...
`GET /api/spaces/:id/packages` filters installed entries by the caller's type-specific read permissions, with the credential ceiling applied, and `GET /api/spaces/:id/packages/:scope/:name` gates on `spaces:read` **and** the row's own type `:read` — the detail route can never answer for a type the listing hides. `GET /api/library` is the organization administration surface: only effective organization owners and admins may access it; members, guests and API keys receive 403. It returns accessible spaces only. A package and its per-space metadata require type-specific read permission in at least one accessible space where the package is PLACED — homed there or shared there (§6.9); placements in spaces the caller cannot read are omitted. System packages remain available with that read permission. Org owners/admins may also manage uninstalled org catalog entries, subject to credential permissions; space-pinned API keys do not gain this org catalog exception. `GET /api/spaces/:spaceId/library` supplies package discovery to space readers, and its candidates are exactly what the caller could put in that space, so the listing never proposes a package the install route would refuse: for a TEAM destination, what is already placed there, plus what the caller may place — a package whose home they hold `<type>:share` in (`home_shareable`), the organization catalogue when they administer it, and system packages; for a PERSONAL destination, the placed alone plus system packages other than integrations, because an offer into somebody's own space is somebody else's act. Installation state and pending offers are restricted to the requested space, and a pending offer appears in the `shared` section ONLY — never also as a matrix entry, so one install never sits behind two buttons. `GET /api/library` carries no `shared` section at all: an offer is addressed to a space, and it is that space's library that presents it. Package types also require read permission in that target space. The route enforces personal-space privacy and API-key pinning. Space package management uses this local view and retains the type-specific install/uninstall permissions; organization administration is never required.
...
**A package is readable from exactly TWO placements: its HOME, and every space it is SHARED into.** `placementGrantsRead(pkg, sharedIn, readable)` (`apps/api/src/lib/package-access.ts`) is the single statement of that rule, and it takes those two — there is no third parameter and no third answer. The HOME half keeps a draft nobody has offered visible to its author, and keeps one installed only in spaces the author cannot reach visible too; without it write authority would exceed read access, leaving a builder able to `PUT` a package they cannot `GET`. The SHARE half is what makes "Shared with me" a listing at all: an offered package has to be readable — its name, its description, the button that takes it up — before the recipient has decided anything.

An INSTALLATION is deliberately not a third placement. It is the only candidate that would answer "why does this space see this package" without ever consulting `<type>:share`: a builder of B who reads A's package from anywhere would install it into B and hand B a placement A granted to nobody. Installing is the act of TAKING an offer — `installPackage` requires `home ∨ shared` inside its own transaction before it writes a row — so an installation is a placement's consequence and never its source.

The rule has three readers in TypeScript plus ONE mirror in SQL, and they differ only in the set they compare against: the catalog check above, the per-space visibility gate of the package read routes (`isPackageReadableInSpace`), and the library projection that serves both `GET /api/library` and `GET /api/spaces/{id}/library`. The SQL mirror is `placementReadFilter` (`apps/api/src/services/package-items/crud.ts` — beside `listOrgItems` rather than beside `placementGrantsRead`, because `lib/package-access.ts` already reaches `services/space-packages.ts` transitively and importing it back would close a cycle), and it has exactly TWO callers: `listOrgItems`, the skills / integrations / mcp-servers index, and `listReadablePackages`, which is `GET /api/agents`. Two index pages asking the same question carry ONE filter, for the reason the home half exists at all: an agent homed in a space and installed in none is editable there, and a page that did not list it would be write authority with no way to reach the thing. The `activeOnly` narrowing the integration picker uses stays install-only, since the home is not a usable instance. The organization-catalogue exception sitting beside it is `home_space_id IS NULL ∧ managesOrgCatalog` and nothing more: a package the organization owns stays the organization's however it is placed, so sharing one takes nothing away from the catalogue.
...
- RUNNING it, from the same predicate: a package readable at home is not thereby executable there. `GET /api/agents` LISTS by the placement rule and says so per row: `AgentListItem.installed` is that second question's answer — `installedPackagesFilter` / `hasPackageAccess`, an installed `space_packages` row or a system package — so a launcher greys its own control out with the reason instead of discovering the refusal on click. The caller-context hints the chat and `get_me` serve (`listInstalledPackageHints`) read the INSTALLED set and nothing else, because what they describe to a model is what it may invoke.
...
The move RECONCILES the placements it invalidates, in the SAME transaction. Every space that still has the package installed and is not the new home receives the `package_shares` row that now places it, with `shared_by` NULL — nobody offered it; the home did, until this call — and the destination's own share is deleted, for the mirror image of the reason `POST …/shares` answers `409 share_target_is_home`. Those authorless rows are ordinary shares: `GET …/shares` lists them with no author, and `DELETE …/shares/{target}` revokes one exactly like any other, uninstalling with it. Without the reconciliation a move would leave an installation nothing places — still running for a schedule, invisible on every page of the space that runs it, which is the state `scripts/migration/0016` exists to repair and a route must never create.
...
| `POST /api/spaces/{spaceId}/packages`                 | the type's `install` in the target space — waived in the caller's OWN personal space (§3.6) — plus `<type>:share` in the home when the package is not placed there yet | THE install door, for a team space and a personal one alike. A package already placed in the space (homed or shared) is installed as-is. One that is NOT gets the share that places it, created in the SAME transaction as the installation, and only for a caller who holds `<type>:share` in its home (`assertPackageShareAccess`: 404 for an id they cannot reach, 403 for one they can see but may not hand out). Reading A's package from B therefore grants nothing about placing it in B. 404 when neither holds and no share authority is offered — never a 403, since a space id can be private. Audit `package.shared` `{ spaceId, targetKind: "space" }`, written only when the transaction actually created the offer; the association row is the record of the other half. An API key never carries `share`, so it installs the already-placed and nothing else |

**A share is a READ grant and nothing else.** It is one of the two terms of `placementGrantsRead` (§6.9), so its readers are that rule's readers: the catalog check, `isPackageReadableInSpace`, the library projection, and the SQL mirror `placementReadFilter` behind both index pages (`listOrgItems` and `GET /api/agents`). Everything else stays on `space_packages` — execution (`hasPackageAccess`), the per-space model and proxy, the stored input settings, the credential resolution. **No execution path reads `package_shares`**, and the grep that proves it finds the table named only in the share service, the placement loaders, the library and the home move.

**A pending offer has exactly ONE place: the `shared` section of the recipient SPACE's library** (`GET /api/spaces/{id}/library`), carrying the sharer's name and the wording that states the act. It is not also a row in that library's `packages.<type>` matrix — that matrix answers "where is this activated, and where can I activate it", and listing an offer in both put one install behind two different buttons. The criterion is mechanical and narrow: a package whose only placement in the target is that offer is withheld from the matrix; one homed there, or already installed there, is a matrix entry like any other. The ORGANIZATION library (`GET /api/library`) has no `shared` section to move a row into, and its matrix IS the placement map an administrator reads, so nothing is withheld there.

**Installing has one door and one rule**, for a personal space exactly as for a team one (`installPackage`): the package must be HOMED in the target or SHARED into it, read `FOR UPDATE` on the share row inside the installing transaction — so a revoke racing an install makes the install refuse rather than commit an installation nothing backs. Otherwise 404, never a 403, since a space id can be private. The caller who holds `<type>:share` in the home is the one exception, and it is not a second door: `POST /api/spaces/{spaceId}/packages` creates the offer and takes it up in that same transaction, which is the admin's one click from the library. A SYSTEM package takes no share — it is readable in every space by construction. That only the space's owner installs into a personal space needs no check of its own: `resolveSpaceRole` answers `null` for everyone else and the space is `private`.

`enabled` is part of that act rather than a setting beside it. On `PUT /api/spaces/{id}/packages/{scope}/{name}`, `enabled: true` is gated as `install` and `enabled: false` as `uninstall` — the same pair `POST …/integrations/{id}/activate` and `DELETE …/deactivate` spell on their own routes — so the personal-space waiver covers it and a recipient can put down what they took up. `modelId`, `proxyId` and `generationConfig` remain `configure`, a body touching both must clear both gates, and a body touching neither is gated as `configure` so an empty patch cannot become a cheap installed-or-not oracle.

**The installation carries no version.** `space_packages` selects no definition at all: it says the package is present here, with its per-space model, proxy and input settings. Outside its home, a package runs the **latest published version, always** — publishing IS the rollout, the way Copilot Studio, custom GPTs, n8n and Apps Script all work. A recipient cannot repair an agent they do not own, so freezing one on bytes its author has stopped maintaining bought them nothing; the real hazard — a new version demanding an access they have not granted — is refused at the right moment by `412 missing_integration_connection` and its connection offers. Dependency versions never came from the installation either: an agent's manifest ranges resolve against the published catalogue and each run freezes what it resolved.
...
- `stage: "draft"` on `POST /api/runs/remote`.
...
**Importing a bundle whose root is homed elsewhere** follows the install rule rather than bypassing it (`bundle-import.ts`): when the caller holds `<type>:share` in the root's home, the import installs it with the offer, the same transaction the install door uses; when they do not, the response reports `root_installed: false` and the refusal is logged at **warn** — a placement that did not happen is an operator-visible fact, not a debug detail.
...
**On the wire**, `homeWireForCaller` emits a THIRD field beside §6.9's pair: `home_shareable`, the same predicate for `share`. Four acts enforce it — the offer, the audience listing, the revoke, and the install that has to create the offer first — and it is not an alias of `home_writable`, since a custom role may hold either without the other. The SPA gates its "Partager…" action on it, and `GET /api/spaces/{id}/library` uses it to decide which packages it may propose for a team destination at all. Taking up an offer asks for no `share`: that is the recipient's act, authorized by the offer plus their own `install` grant — or, at home, by ownership alone (§3.6).
...
- A key may mutate a package only when its pinned space is the package's HOME and its scopes grant the type's mutation permission (§6.9). Other installations do not add authority or veto it. A NULL home is organization-catalogue authority, which keys never hold.
...
| `POST/PATCH /api/roles`, GRANTING a bundle                         | code in core, **gated** | `features.custom_roles: true` — set by `module-ee`; any EE-licensed module can set it |
| `DELETE /api/roles/{id}`                                           | ✅ never gated          |                                                                                       |
| billing managers, billing contact, `billing:*`                     |                         | `module-ee` (§10)                                                                     |
| `principalPermissions` hook                                        | contract in core        | any module                                                                            |

**What an EE → OSS downgrade does with the bundles already stored** (issue #1334). They keep granting — the resolver never reads the flag, so nobody loses access when a subscription lapses. What the flag stops is every verb that would GROW their reach: defining a bundle, editing one, assigning one to a space member, and naming one in a deferred grant (an invitation, an OAuth client's signup policy — including each replay of a standing policy, which would otherwise keep propagating bundles indefinitely). What it never stops is the verbs that shrink it: moving a holder onto a preset, and `DELETE /api/roles/{id}`. `GET /api/spaces/{id}/roles` answers "what can I assign here", so it stops offering bundles too; `GET /api/roles` keeps listing them, because that is where a leftover is found and deleted. The one grant not re-checked is an invitation accepted after the downgrade: it was authored under licence, it expires on its own, and refusing it would strand an invitee with no way to fix it.
