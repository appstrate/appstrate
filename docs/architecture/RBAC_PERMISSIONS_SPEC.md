...
`GET /api/spaces/:id/packages` filters installed entries by the caller's type-specific read permissions, with the credential ceiling applied, and `GET /api/spaces/:id/packages/:scope/:name` gates on `spaces:read` **and** the row's own type `:read` — the detail route can never answer for a type the listing hides. `GET /api/library` is the organization administration surface: only effective organization owners and admins may access it; members, guests and API keys receive 403. It returns accessible spaces only. A package and its installation metadata require type-specific read permission in at least one accessible installation; hidden installations are omitted. System packages remain available with that read permission. Org owners/admins may also manage uninstalled org catalog entries, subject to credential permissions; space-pinned API keys do not gain this org catalog exception. `GET /api/spaces/:spaceId/library` supplies package discovery to space readers, including candidates readable in other accessible spaces for team destinations (personal spaces only accept explicit shares), with installation state and pending offers restricted to the requested space. Package types also require read permission in that target space. The route enforces personal-space privacy and API-key pinning. Space package management uses this local view and retains the type-specific install/uninstall permissions; organization administration is never required.
...
REST runs and scheduled runs with no explicit version selector execute the installed pin; export and CLI already use the same precedence. Dashboard launch forms and readiness default to that snapshot too, falling back to the draft only for an unpinned installation. An explicit draft, published or version selector remains a deliberate override. Deleting a version pinned by any installation returns `409 version_in_use`, so deletion cannot silently change an accepted share into a follow-latest installation.
