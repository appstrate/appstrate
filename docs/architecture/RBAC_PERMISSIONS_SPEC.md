...
| `viewAs`         | `resolveViewAs`, before every `permissions` write | Validated role preview (`X-View-As`), or unset. Narrows the two sets above to a lesser persona's; `user` and `orgRole` stay REAL. See `docs/architecture/RBAC_VIEW_AS_PLAN.md` |
