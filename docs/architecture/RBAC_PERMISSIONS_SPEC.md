...
| `roles`                                           | `read`, `write`, `delete`                 | **New.** Custom space-role definitions. `read` is held by every org role except `guest` — delegated space administrators use the filtered space catalog (§6.4), including org guests. `write`/`delete` are owner/admin and additionally require `features.custom_roles`. |
...
| `agents`                | `read`, `write`, `configure`, `delete`, `run`                                           | Authoring happens from a space; the package row is org-scoped but reachable only through `space_packages`. `configure` also gates INSTALL/config/uninstall of an agent in a space (`POST`/`PUT`/`DELETE /api/spaces/:spaceId/packages`) — installing decides which space runs an agent, it does not author one. Shared drafts and versions require the relevant mutation permission in every installed space (§6.8). |
...
Guards become `requirePermission("members", …)`; the assignable-role policy keeps running in the handler. `role` accepts `admin | member | guest`. Invite body gains `space_assignments`. Guests require at least one; admins require an empty list; members may carry explicit grants. Space and custom-role references must belong to the organization. Acceptance applies grants in the membership transaction; deleted targets are skipped and logged.
...
- **Role preview** ("view as", §6.7) is entered from two places, both visible only to an owner or an administrator (`useCanPreviewRole()`): a **Prévisualiser** action on every row of Org settings → Roles, presets and custom bundles alike, and a **Voir en tant que…** action beside "Ajouter un membre" on Space settings → Members. Both open one dialog (`components/view-as-dialog.tsx`) — the org role as described radio options, the space, and that space's grantable role catalog (`GET /api/spaces/:id/roles`, read with the caller's REAL permissions, before the preview starts). Space and role are optional as a pair, matching the header. While a preview runs, a permanent warning banner above the page header names the persona and the space and carries the only exit; it is not dismissible. Leaving, switching organization and losing the session all drop it, and so does a refusal (§6.7), with a translated reason.
...
**Schema — `packages/db/drizzle/0056_space_roles.sql`:** `ALTER TYPE org_role ADD VALUE 'guest'`; `space_roles`, `space_members`; `spaces.visibility`/`default_role` + checks; `org_invitations.space_assignments`; `chat_sessions.space_id`; `oauth_clients.signup_space_assignments`. Two writes ride along, each licensed by a constraint the same file promotes on the same table (§2 of the doctrine):

- `chat_sessions.space_id` is backfilled to the org's default space, then `SET NOT NULL`.
- Legacy OAuth `signup_role = 'viewer'` clients snapshot a viewer grant in every currently existing org space into `signup_space_assignments`, preserving existing explicit choices, before becoming `guest`. Then `oauth_clients_signup_role_check` is re-added narrowed to `admin | member | guest`.
...
3. Pending viewer invitations snapshot the same current-space viewer grants into `space_assignments`, preserving explicit choices, then become `guest`. Acceptance retains their former reach; spaces created later never enter the snapshot, including after a migration rerun.
4. Verification that **discriminates**: the counts of `org_members.role = 'viewer'` and of pending `viewer` invitations must both be 0 **and** every pre-flip (user, space) pair must be covered by a `space_members` row, and every pending invitation must carry its snapshot grants — printed before and after, and raised on rather than returned.
...
4. **Verify.** `0008` raises rather than returns: zero `org_members.role = 'viewer'`, zero pending `viewer` invitations, every former (user, space) pair covered by a `space_members` row, and every captured pending invite carrying its space snapshot. `0056` must also leave no OAuth `signup_role = 'viewer'`; inspect the stored assignment snapshots captured during rehearsal.
