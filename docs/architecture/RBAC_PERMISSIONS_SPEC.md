...
| `agents`                | `read`, `write`, `configure`, `delete`, `run`                                           | Authoring happens from a space; the package row is org-scoped but reachable only through `space_packages`. Deleting a package installed in another space stays whatever the service does today — not a permission question. |
| `skills`, `mcp-servers` | `read`, `write`, `delete`                                                               | same                                                                                                                                                                                                                        |
| `integrations`          | `read`, `write`, `delete`, `install`, `uninstall`, `configure`, `connect`, `disconnect` | **New `configure`** = OAuth clients + per-space defaults (`integrations.ts:1037-1178`). Session-only, never API-key-grantable — this is what retires `assertOrgAdmin`.                                                      |
...
`principalPermissions(ctx: { orgId, userId }) → Promise<readonly string[]>` is awaited once per request in the pipeline, for session callers only. It is not evaluated for API keys or end-user tokens: a module may only declare session-only strings in `mayGrant` (never `apiKeyGrantable` / `endUserGrantable` ones), so those ceilings could not contain the result anyway. Results are cached with the `@appstrate/core/cache` primitive under `(orgId, userId)` and invalidated through the pg_notify bus on the module's own writes — a module that contributes this hook owns its invalidation, the same way cloud owns its billing cursor. Strings a module returns are validated against `ORG_LEVEL_PERMISSIONS` at boot-time registration of the hook's declared `mayGrant` list; an undeclared string at runtime is dropped and logged.
...
| Method   | Path                              | Permission                  | Notes                                                                                                                                                    |
| -------- | --------------------------------- | --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`    | `/api/spaces/:id/members`         | `space-members:read`        | explicit rows + implicit members (`source: "explicit" \| "org_role" \| "open_space"`), so the page shows who actually has access, not just who was added |
| `POST`   | `/api/spaces/:id/members`         | `space-members:invite`      | `{ user_id, preset_role }` or `{ user_id, custom_role_id }`; user must be an org member; 409 for owner/admin                                             |
