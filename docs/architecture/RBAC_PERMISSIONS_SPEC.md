...
The same union is what `GET /api/orgs` and `GET /api/me/orgs` expose in their `permissions` field, and what the `/api/orgs/:orgId/*` path resolver writes — one helper (`listedOrgPermissionsForCaller`, `apps/api/src/lib/principal-permissions.ts`) behind all of them, so the listing and the guard cannot disagree.
