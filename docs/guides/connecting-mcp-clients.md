...
| `pull_package_files` | `mcp:read`                    | Every file of a skill's draft (or a published version with `version`), text inline and binaries in base64, plus the draft's `lock_version`.                                                                                                                                                                                       |
| `package_status`     | `mcp:read`                    | Compare files you hold to the skill's draft: modified / added / removed, and whether the draft moved since your `lock_version`.                                                                                                                                                                                                   |
| `push_package_files` | `mcp:invoke` + `agents:write` | Write the COMPLETE file set to the skill's draft without publishing (`POST /api/packages/import?draft=true` in-process); `lock_version` proves the draft is what you last saw, `force` overrides `draft_overwrite`, `publish: true` cuts a version instead, `space` installs the draft in that space rather than the default one. |
