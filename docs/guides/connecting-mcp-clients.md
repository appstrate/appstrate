...
| `push_package_files` | `mcp:invoke` + `agents:write` | Write the COMPLETE file set to the skill's draft without publishing (`POST /api/packages/import?draft=true` in-process); `lock_version` proves the draft is what you last saw, `force` overrides `draft_overwrite`, `publish: true` cuts a version instead. |
