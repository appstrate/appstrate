 ...
 * re-generating).
 *
 * When `AUTH_BOOTSTRAP_OWNER_EMAIL` is absent the file ends with a
 * 3-line pointer to the closed-mode docs — pure discoverability for
 * self-hosters who'll edit the `.env` anyway. When the bootstrap email
 * IS set, the lockdown is already configured, so the pointer is
 * suppressed (no point telling the user to read about a feature
 * they've already enabled).
 */
export function renderEnvFile(env: EnvVars): string {
  const lines = Object.keys(env)
    .sort()
    .map((key) => `${key}=${env[key]}`);
  const closedModeFooter = env.AUTH_BOOTSTRAP_OWNER_EMAIL
    ? ""
