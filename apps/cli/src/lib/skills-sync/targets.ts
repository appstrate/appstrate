...
installed in the space(s) your Appstrate CLI profile syncs.
...
/**
 * Where the plugin tree of ONE profile lives. The shared roots are the user's
 * (one `~/.agents/skills` for every profile, ownership settled by the ledger),
 * but a plugin tree is swapped whole, so two profiles cannot share one: the
 * default profile keeps `claude-plugin/`, the marketplace path, and every other
 * profile gets `claude-plugin-<profile>/`.
 */
export interface Layout {
  pluginRoot: string;
}

export function layoutFor(profileName: string, defaultProfile: string): Layout {
  const dir = profileName === defaultProfile ? "claude-plugin" : `claude-plugin-${profileName}`;
  return { pluginRoot: join(getDataDir(), dir) };
}

/** `homeDir()` is the helper `getDataDir()` uses, so the ledger cannot disagree. */
export function targetRoot(target: SyncTarget, layout: Layout): string {
  switch (target) {
    case "claude-plugin":
      return layout.pluginRoot;
    case "codex":
...
export function skillDir(target: SyncTarget, slug: string, layout: Layout): string {
  const root = targetRoot(target, layout);
...
export async function writeSharedSkill(
  ...
  await withStaging(
    skillDir(target, tree.slug, layout),
    join(root, STAGING_DIR),
    async (staging) => {
...
export async function destinationExists(
  ...
  try {
    await lstat(skillDir(target, slug, layout));
    return true;
  } catch {
    return false;
  }
