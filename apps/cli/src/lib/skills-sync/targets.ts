...
const PLUGIN_MANIFEST = {
  ...
  description:
    "Skills and agent commands from your Appstrate organization, kept in sync by `appstrate code sync`.",
};

const PLUGIN_README = `# Appstrate skills

This directory is generated by \`appstrate code sync --target claude-plugin\`
from the skills installed in the spaces selected on your Appstrate CLI
profile, and from the agents active in its pinned space.
...
Skills appear in Claude Code as \`/${PLUGIN_NAME}:<skill>\`, agents as
\`/${PLUGIN_NAME}:run-<agent>\`: each one launches that agent in the pinned
space through the bundled MCP server.
`;
...
the active pinned space, independently of the spaces supplying the skills. Open \`/mcp\` in Claude Code to connect
...
async function withStaging(
  ...
  try {
    ...
    // The swap has committed. Cleanup failure must not report the old tree as
    // current and leave its ownership in the ledger; finally retries cleanup.
