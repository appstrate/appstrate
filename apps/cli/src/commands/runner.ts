...
function printPostInstall(config: RunnerConfig, source: TokenSource, healthy: boolean): void {
  ...
  clack.note(
    [
      ...
      `  FIRECRACKER_RUNNER_ALLOW_PLAINTEXT=1   # same-host only — plaintext never leaves the machine`,
      ``,
      `Open the daemon port for the platform:`,
      ...fw.commands.map((c) => `  ${c}`),
      ``,
      `Platform on a DIFFERENT host? Drop the ALLOW_PLAINTEXT=1 line, put TLS in front of`,
      `this daemon (reverse proxy) and use https:// in FIRECRACKER_RUNNER_URL — the wire`,
      `carries run credentials, and the platform refuses plaintext split-host by default.`,
      ``,
