...
function printPostInstall(config: RunnerConfig, source: TokenSource, healthy: boolean): void {
  ...
  clack.note(
    [
      ...
      `  FIRECRACKER_RUNNER_TLS_REQUIRED=0   # same-host only — plaintext never leaves the machine`,
      ``,
      `Open the daemon port for the platform:`,
      ...fw.commands.map((c) => `  ${c}`),
      ``,
      `Platform on a DIFFERENT host? Drop the TLS_REQUIRED=0 line, put TLS in front of`,
