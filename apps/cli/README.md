...
| `--space <ref>`   | Repeatable. A space to sync, by id or exact name. The run materializes the **union** of the spaces given; a package installed in several of them is read from the first one named. Without the flag: the pinned space plus the profile's `syncSpaces` list in `config.toml` (ids only) — the persisted form, since a marketplace command string cannot carry the flag. |
...
**Several profiles on one machine.** The ownership ledger is kept per profile: a profile pointed at another organization (or another space) can sync into the same `~/.agents/skills` without the two deleting each other's skills — each one only ever removes what it recorded itself, and a slug the other profile owns is refused like any hand-written directory. The plugin tree is the exception, since it is swapped whole: the default profile keeps `claude-plugin/` (the marketplace path), every other profile gets its own `claude-plugin-<profile>/`. A ledger written by a CLI that predates this is adopted by the first profile that runs.
...
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `skills pull <skill> [dir]`    | Brings the skill's files into a local working folder (`./<name>` by default): the draft, or a published version with `--version <semver\|latest>`. `manifest.json` comes along so a later `push` keeps the id and version; the draft's `lock_version` at pull time is recorded so that push needs no `--force`. Refuses a folder that already has files unless `--force`.                                                                                                                                                                   |
| `skills push <dir>`            | Zips the folder (every file under it except `.git`, `node_modules`, `.DS_Store` and the like) and sends it to `POST /api/packages/import?draft=true`. A `manifest.json` in the folder is used as authored (its `version` is moved to the next patch when it is already published, which is what a folder from `pull` carries); otherwise one is synthesized from the `SKILL.md` frontmatter as `@<org slug>/<name>`, versioned by the frontmatter `version:` if present, else a patch bump over the latest published version, else `1.0.0`. |
| `push --id @scope/name`        | Push under this package id instead.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `push --force`                 | The server refuses to replace a draft that was edited elsewhere since this machine last pushed it (`409 draft_overwrite`); this overrides it. Re-pushing your own work never needs it: `push` keeps the `lock_version` the server returned (under `$XDG_DATA_HOME/appstrate/skills-push/<profile>.json`) and sends it back, which satisfies the protection when the draft is still exactly what you last pushed.                                                                                                                            |
| `push --dry-run`               | List the entries and the id/version that would be sent; send nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `skills status <dir-or-skill>` | Compares the folder to the skill's draft, file by file (`M` modified, `A` added, `D` removed; `--diff` prints line diffs for text files), and says whether the draft was edited elsewhere since this machine last pulled or pushed it. Computed when you run it — no watcher, no local database: the draft is the baseline. `push` prints the same status first and refuses a folder that matches the draft.                                                                                                                                |
