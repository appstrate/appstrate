...
**Directory names.** The Agent Skills spec requires the frontmatter `name` to equal the parent directory name, and Appstrate ids are `@scope/name`, which is not a legal skill name. The directory is therefore the frontmatter `name` when it is already legal, falling back to the slugified package `name` segment. A name already installed stays with its package for as long as the catalogue still lists that package, resolved on this run or not: the package takes its preferred name when that name is free, and otherwise keeps the one it has. A name changes owner only when its holder leaves the catalogue or renames itself (a new frontmatter `name`); the old name is then free, and another package may take it at a later sync — a run's resolution order never moves it. A newcomer claiming a held slug becomes `<scope>-<name>`, then `<scope>-<name>-2`, `-3`, … until the name is free (newcomers are ordered by package id, so the choice is reproducible). Every rename is reported on stderr.
...
With the `claude-plugin` target, sync also installs one command per agent ACTIVE in the **pinned** space, system agents included. Type it with your request; Claude collects the input, launches the agent through the plugin's MCP server with `run_and_wait`, waits for the run and reports its result with every file it produced:

```text
/appstrate:run-invoice-extractor extract the totals from ~/Downloads/march-invoice.pdf
```

**Which agents, and who gets them.** Agents come from the pinned space alone: `--space` and `syncSpaces` select skill sources and never change it. An MCP session runs in exactly one space — the one in `X-Space-Id` — and the agent's input settings, the run and any file you upload all belong to it. You must be a member of the pinned space with `agents:run`, `runs:read` (or `runs:read-all`) and `mcp:invoke` there — exactly the grant on which the MCP server offers `run_and_wait` and `invoke_operation`, and which the `runner` role has; otherwise sync installs no agent command and says so on stderr at every sync. After `appstrate space switch`, the next sync replaces the commands with the new space's agents. `codex` and `claude-user` never receive agent commands: nothing configures the MCP server a command calls there.

**What a command contains.** Two files, `skills/run-<agent>/SKILL.md` and `skills/run-<agent>/input.json`. `input.json` is the space's launch contract — the same split the dashboard's launch form makes: the input `schema`, and which top-level fields are `prompted` (taken from your request; Claude asks only for a missing required one, leaves out the others and never invents a value), `prefilled` (a default or a value the space stores; sent only when you explicitly override it) and `locked` (never sent). Stored values themselves are never written to disk, only which fields have one. `argument-hint` lists the prompted fields, `<required>` and `[optional]`. The `SKILL.md` body is generated text plus the agent id and version (the space id is in the frontmatter `metadata`): the agent's name, description and field names live only in `input.json` and in escaped frontmatter, because Claude Code expands `$…` placeholders and shell commands in a skill body. It names every tool in full (`mcp__plugin_appstrate_appstrate__run_and_wait`, …), so a second Appstrate MCP server exposing the same bare names is never called by mistake. `disable-model-invocation: true` means Claude never launches an agent on its own — only you do, by typing the command.

**The run asks for permission.** A command pre-approves nothing, so reading `input.json` may prompt too. The `run_and_wait` call is not pre-approved: Claude Code's permission prompt shows the exact `input` before the metered run starts, and approving it is the confirmation — unless you allowed the tool permanently or run in an auto-accept mode. A connection problem (`connect_url`, `must_choose_connection`) is handled as the Appstrate MCP server's own instructions say. A `404` other than the out-of-date codes below, or a `400`, is an input problem — an unreadable file URI, say — that Claude fixes with you before retrying; anything else is reported. `run_and_wait` is retried only for those refusals, never once a run id exists, so one command costs at most one run: a run still going when `run_and_wait` stops waiting (30 minutes) comes back `done: false`, Claude keeps waiting on it with `getRun`, then lists its files with `list_files`.

**File inputs.** A file field takes a URI, never inline content. For a local file Claude calls `createUpload` through `invoke_operation` (reading only its body shape with `describe_operation`), `PUT`s the bytes to the returned URL with `curl` and the returned headers, and passes the returned `upload://` URI. Keep the file inside the session's working directory, or start Claude Code with `--add-dir <dir>` for the folder holding it: Claude Code restricts shell access to its working directories, so a file elsewhere either prompts for permission or gets copied through the model's context, which spends tokens and may corrupt a binary file. A file an earlier run produced is passed as its `appfile://` URI (`list_files` lists them). Uploads belong to the pinned space, where the run executes.

**Keeping commands current.** A command pins the version it launches: `--source published` writes the agent's latest published version, `--source draft` writes `draft`; as for skills, an agent you cannot write is reported and removed, and a system agent always uses its published version. Nothing is downloaded: the command is generated from the agent's definition and the space's settings, so a republished agent, a changed schema, lock or default, or another pinned space changes its bytes and the next sync rewrites it. Claude Code re-runs the sync each session; `claude plugin update appstrate@appstrate` applies it now. A command that has fallen behind — the agent deactivated in that space, deleted, or its pinned version gone — gets a `404` `agent_not_active_in_space`, `agent_not_found` or `no_published_version` (or a pinned version not found) at launch, and tells you to run `appstrate packages sync`. A command the generator refuses to render (a server value that fails its identifier checks) is reported on stderr and removed, not kept.

**Names.** `run-<name>`, from the package name, under the same rule as skill directories: an installed command keeps its name while its agent is still active in the space, and passes to another package only after its agent leaves — so a command does not quietly start launching, and billing, a different agent. A newcomer whose `run-<name>` is taken becomes `run-<scope>-<name>`, then `run-<scope>-<name>-2`, `-3`, …, reported on stderr like a skill rename. Skills are named before agents, so an agent never renames an installed skill; a brand-new skill whose name an installed agent command already holds falls back like any newcomer.
...
- **By package id** — `@scope/agent[@spec]`. Runs **remotely** by default: the CLI POSTs `/api/agents/{scope}/{name}/run` on the pinned instance (the same path as the dashboard "Run" button), then tails the run's logs and final status. Pass `--local` to run the agent in-process instead — the CLI then calls `GET /api/agents/{scope}/{name}/bundle` to download a deterministic `.afps-bundle`, verifies its SRI integrity in memory, and runs it via the Pi runner. The bundle is the run: `@draft` downloads the author's working copy as the ROOT with every dependency at the published version its manifest range selects — the closure a server-side draft run resolves — so a local run and a run on the instance execute the same bytes, and a dependency no published version satisfies fails the download with the same `422 dependency_unresolved` the run answers. The bytes are never written to disk — every invocation re-fetches.
...
```sh
# Run the latest version of an agent active in the pinned space
...
# Run the author's working copy — only for people who can write the package
appstrate run @scope/triage@draft
...
An id with no `@spec` runs the **latest published version**, on this surface as on every other. `draft` and `published` are the two reserved selectors: `@draft` runs the author's working copy and the instance answers `403 draft_not_writable` to anyone who cannot write the package, `@published` spells the default out loud. Neither can collide with a dist-tag: `latest` is the only dist-tag the platform ever writes, auto-managed at publication, and there is no route through which anyone creates another.

Run-config inheritance (model, proxy, stored inputs) is fetched from `/api/spaces/{spaceId}/packages/{scope}/{name}/run-config` and merged with flag/env overrides. Use `--no-inherit` to opt out (deterministic CI). The version is not part of it: a space holds no version of its own, so nothing about the version is inherited.
