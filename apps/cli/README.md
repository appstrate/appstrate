...
**Directory names.** The Agent Skills spec requires the frontmatter `name` to equal the parent directory name, and Appstrate ids are `@scope/name`, which is not a legal skill name. The directory is therefore the frontmatter `name` when it is already legal, falling back to the slugified package `name` segment. A name already installed stays with its package for as long as the catalogue still lists that package, resolved on this run or not, so a slash command never silently switches to a different package. A newcomer claiming the same slug becomes `<scope>-<name>`, then `<scope>-<name>-2`, `-3`, … until the name is free (newcomers are ordered by package id, so the choice is reproducible); once the holder leaves, the name moves to the newcomer at the next sync. Every rename is reported on stderr.
...
**Names.** `run-<name>`, from the package name, under the same rule as skill directories: an installed command keeps its name while its agent is still active in the space — a command must never silently launch, and bill, a different agent — and a newcomer whose `run-<name>` is taken becomes `run-<scope>-<name>`, then `run-<scope>-<name>-2`, `-3`, …, reported on stderr like a skill rename. Skills are named before agents, so an agent never renames an installed skill.
