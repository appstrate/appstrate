...
**Which agents, and who gets them.** Agents come from the pinned space alone: `--space` and `syncSpaces` select skill sources and never change it. An MCP session runs in exactly one space — the one in `X-Space-Id` — and the agent's input settings, the run and any file you upload all belong to it. You must be a member of the pinned space with both `agents:read` and `agents:run` there; otherwise sync installs no agent command and says so once on stderr. After `appstrate space switch`, the next sync replaces the commands with the new space's agents. `codex` and `claude-user` never receive agent commands: nothing configures the MCP server a command calls there.

**What a command contains.** Two files, `skills/run-<agent>/SKILL.md` and `skills/run-<agent>/input.json`. `input.json` is the space's launch contract — the same split the dashboard's launch form makes: the input `schema`, and which top-level fields are `prompted` (taken from your request; Claude asks for any that is missing and never invents one), `prefilled` (a default or a value the space stores; sent only when you explicitly override it) and `locked` (never sent). Stored values themselves are never written to disk, only which fields have one. `argument-hint` lists the prompted fields, `<required>` and `[optional]`. The `SKILL.md` body is generated text plus the agent id, version and space id: the agent's name, description and field names live only in `input.json` and in escaped frontmatter, because Claude Code expands `$…` placeholders and shell commands in a skill body. `disable-model-invocation: true` means Claude never launches an agent on its own — only you do, by typing the command.

**The run asks for permission.** A command pre-approves one thing: reading its own `input.json`. The `run_and_wait` call is not pre-approved, so Claude Code's permission prompt shows the exact `input` before the metered run starts — approving it is the confirmation. A run that needs a connection returns the link, or a choice between connections that Claude asks you about; a run still going when `run_and_wait` stops waiting (30 minutes) comes back `done: false`, and Claude keeps waiting with `getRun`.

**File inputs.** A file field takes a URI, never inline content. For a local file Claude requests an upload (`createUpload` through `invoke_operation`), `PUT`s the bytes to the returned URL with `curl` and the returned headers, and passes the returned `upload://` URI. A file an earlier run produced is passed as its `appfile://` URI (`list_files` lists them). Uploads belong to the pinned space, where the run executes.

**Keeping commands current.** A command pins the version it launches: `--source published` writes the agent's latest published version, `--source draft` writes `draft` (which, as for skills, needs write access to the agent; a system agent always uses its published version). Nothing is downloaded: the command is generated from the agent's definition and the space's settings, so a republished agent, a changed schema, lock or default, or another pinned space changes its bytes and the next sync rewrites it. Claude Code re-runs the sync each session; `claude plugin update appstrate@appstrate` applies it now. A command that has fallen behind — the agent deactivated, deleted or never published in that space — answers `404`, and tells you to run `appstrate packages sync`.
