...
/**
 * Preview-token + HTML-hardening primitives for the cookie-less file
 ...
 *    tokens authorizing a GET of ONE file's preview for 5 minutes. They are
 ...
 * A larger file is rejected with 413 rather than streamed unmodified.
 */
...
interface PreviewTokenPayload {
  /** File id the token authorizes a preview of. */
  d: string;
  /** Org the file belongs to (binds the token to a tenant). */
  o: string;
  /** Expiration unix timestamp (seconds). */
  e: number;
  /**
   * Minting actor's dashboard-user id (null for an end-user actor). Bound so
   * the route can re-check a `user_upload` preview against the file's
...
/**
 * The four ways a file can be previewed in-browser (or null for "not
 ...
 * docs/architecture/FILES.md → "Preview kinds".
 */
...
/**
 * Classify a file's mime into its {@link PreviewKind}, or null when it is
