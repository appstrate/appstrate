...
/** Package discovery with either organization-wide or single-space installation state. */
export async function getPackageLibrary(c: Context<AppEnv>, spaceId?: string) {
  const orgId = c.get("orgId");

  const accessible = await packageAccessSpaces(c);
  // Discovery retains packages readable elsewhere, so a space builder can install
  // them into a team space. A personal space only accepts explicit shares.
  ...
  const [rows, shared] = await Promise.all([
    ...
    listSharedNotInstalled(visibleSpaces.map((space) => space.id)),
  ]);
  ...
  for (const row of rows) {
    ...
    if (!placed && !systemAvailable && !orgCatalogEntry) continue;
