...
- Shortcut — the \`run_and_wait\` tool launches a run AND follows it in one call: pass \`kind: "agent"\` with \`scope\`/\`name\` (+ optional \`version\`/\`input\`), or \`kind: "inline"\` with \`manifest\`+\`prompt\`. It long-polls until terminal and returns \`{ id, status, done, result?, error? }\`. Prefer it over a manual trigger-then-poll for a single run you intend to wait on. If it returns \`done: false\` the run is still going — its live logs already stream in the chat; keep following it with the run get operation (\`wait: true\`) and read \`result\` when it finishes.
