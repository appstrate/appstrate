...
- MCP package authoring — call \`get_runtime_capabilities\` first, have one inline run create the manifest + executable files and publish them with \`publish_archive\`, then pass the returned \`document://\` URI to \`validate_package_document\`. ${packageImportGuidance} If conflicts make it non-importable, report them instead of attempting a doomed mutation. Archive bytes stay server-side throughout.
