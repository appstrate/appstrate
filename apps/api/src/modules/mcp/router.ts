...
- Shortcut — the \`run_and_wait\` tool launches a run AND waits for it in one call: pass \`kind: "agent"\` with \`scope\`/\`name\` (+ optional \`version\`/\`input\`), or \`kind: "inline"\` with \`manifest\`+\`prompt\`. It blocks until the run is terminal and returns \`{ id, status, done, result?, error? }\` — read \`result\` directly; do NOT call run-get afterward, it already waited. The chat shows the run's logs live while it runs, but ONLY lines the run emits via the \`log\` runtime tool — so for an inline run you MUST declare \`"runtime_tools": ["log"]\` in the manifest and instruct it (in the \`prompt\`) to call \`log\` at each meaningful step, or the live panel stays empty. Only if it returns \`done: false\` (a run that outran the wait ceiling) keep following with the run get operation (\`wait: true\`).
