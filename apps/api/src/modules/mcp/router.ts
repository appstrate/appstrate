...
- Shortcut — \`run_and_wait\` launches a run, exposes the created run to chat for live progress, then waits internally and returns \`{ id, packageId, status, done:true, result?, error? }\` once the run is terminal. Do NOT call \`getRun\` after \`run_and_wait\` just to wait for completion; this tool already waits. The chat shows ONLY lines the run emits via the \`log\` runtime tool — so for an inline run you MUST declare \`"runtime_tools": ["log"]\` in the manifest and instruct it (in the \`prompt\`) to call \`log\` at each meaningful step, or the in-chat run progress component stays empty.
