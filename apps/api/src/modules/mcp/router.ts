...
- Code \`duplicate_connection_label\` on \`integrations.<id>\` is a connection error you cannot fix yourself: the connections bound to that integration share a label, and the label is how a tool call names its connection. Do not rename, re-pick or drop one — ask the user to rename one of the connections it lists, then retry.
- Code \`auth_serves_no_selected_tool\` on \`integrations.<id>\` is not a connect problem either: the connection its \`connection_id\` names was explicitly bound (your \`connection_overrides\`, or a pin or default) and was made on an auth that exposes none of the agent's selected tools, so reconnecting it changes nothing. When you passed \`connection_overrides\`, retry without that id; when a pin or default binds it, tell the user which connection to take out of the set. Carrying no \`connection_id\`, it means the agent's own \`auth_key\` excludes every auth that serves its selected tools: tell the user the agent's configuration must change.`
    : "";
