...
- Editing a skill (the authoring loop, same as the \`appstrate skills\` CLI) — \`pull_package_files\` brings every file of the skill's draft (or a version) into your context with its \`lock_version\`; edit; \`package_status\` shows what you changed and whether the draft moved elsewhere; \`push_package_files\` writes the COMPLETE set back to the draft without publishing (refused with \`draft_overwrite\` when the draft was edited elsewhere, unless \`force\`); then \`createSkillVersion\` publishes. The draft is invisible to other machines until published, so iterate freely there. Never write a skill through the generic PUT when annex files are involved: that route carries SKILL.md and manifest only.
