...
- MCP package authoring — call \`get_runtime_capabilities\` first, have one inline run create the manifest + executable files and publish them with \`publish_archive\`, then pass the returned \`document://\` URI to \`validate_package_document\`. Call \`import_package_document\` only when the user asked to add/install the valid package. Archive bytes stay server-side throughout.
