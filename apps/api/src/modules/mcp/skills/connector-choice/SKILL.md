...
2. **Sinon, la variante API simple** (`source.kind: "local"`, `auths: oauth2 | api_key`) → utilise-la.
   - OAuth : déclenche le flux de connexion natif (il rend un **bouton one-click** dans le chat —
     ne fabrique jamais une URL OAuth à la main).
   - Clé API : la saisie d'un secret ne se fait pas dans le chat — dirige l'utilisateur vers la page
     Intégrations de l'app pour y connecter le service. Attention : beaucoup d'API forcent l'utilisateur
...
OAuth one-click** dans le chat pour les services OAuth, ou la **page Intégrations** de l'app pour une
clé API.
