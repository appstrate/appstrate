...
describe("readResponse — non-text bodies", () => {
  ...
  it("decodes a small UTF-8 text attachment without changing its HTTP media type", async () => {
    const text = "# Triage\n\nMéthode sélective.";
    const response = new Response(text, {
      ...
      headers: {
        ...
        "content-disposition":
          "attachment; filename=\"triage-sentiment.md\"; filename*=UTF-8''triage-sentiment.md",
      },
    });
  ...
  it("keeps an invalid UTF-8 text attachment on the binary-safe path", async () => {
    const response = new Response(new Uint8Array([0xff, 0xfe]), {
      ...
      headers: {
        ...
        "content-length": "2",
        "content-disposition":
          "attachment; filename=\"reference.md\"; filename*=UTF-8''reference.md",
      },
    });
    const body = parse(await readResponse(response));
    expect(body.note).toBe("Non-text response body omitted.");
  ...
  it("does not buffer a text attachment above the response ceiling", async () => {
    const response = new Response("not read", {
      ...
      headers: {
        ...
        "content-disposition": "attachment; filename=\"large.md\"; filename*=UTF-8''large.md",
      },
    });
    const body = parse(await readResponse(response));
    expect(body.note).toBe("Non-text response body omitted.");
