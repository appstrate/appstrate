 ...
 * `enforceResourceAudience` middleware + protected-resource registry.
 *
 ...
 * isolation — INBOUND (a resource path requires its URI in the audience) and
 * OUTBOUND (a resource-bound token may not be replayed on other routes, except
 * for the in-process self-dispatch). The "a real minted token carries the right
 * aud" half is covered in the oidc oauth-flows + dcr-cimd suites.
 */
...
describe("enforceResourceAudience — inbound (RFC 8707)", () => {
  ...
  it("allows a caller with no token audience (cookie / API key — first-party)", async () => {
    expect((await req("/api/mcp")).status).toBe(200);
  });

  it("allows a token whose aud includes the resource URI", async () => {
    expect((await req("/api/mcp", [mcpUri])).status).toBe(200);
  });

  it("allows when the resource URI is one of several audiences", async () => {
    expect((await req("/api/mcp", ["https://other.example", mcpUri])).status).toBe(200);
  });

  it("rejects a token bound to a different resource with 401", async () => {
    expect((await req("/api/mcp", ["https://other.example"])).status).toBe(401);
  });

  it("rejects a token with an empty audience array with 401", async () => {
    expect((await req("/api/mcp", [])).status).toBe(401);
  });

  it("matches sub-paths of the resource prefix", async () => {
    expect((await req("/api/mcp/anything", [mcpUri])).status).toBe(200);
    expect((await req("/api/mcp/anything", ["https://other.example"])).status).toBe(401);
  });
});
