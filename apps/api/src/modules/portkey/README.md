...
- **Phase 4b — fallback chains** — multi-provider failover via `targets[]` in the inline config (e.g. anthropic → bedrock on rate-limit). Requires a UI/data model for "primary + fallback model preset" pair; deferred behind Phase 3 since the open model catalog is a prerequisite.
