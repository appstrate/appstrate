...
- ✅ **Phase 2.5 — Portkey mandatory** shipped. Module added to default `MODULES`, fail-fast at boot via `assertPortkeyRoutersInstalled()`, fallback branches removed from `run-launcher/pi.ts` and `llm-proxy/core.ts`. `accept-encoding: identity` injected unconditionally on proxy requests (works around Bun fetch ZlibError on Anthropic SSE — discovered in real-key smoke). Follow-up cleanup: the `portkeyRouting ? : direct-upstream` ternaries were dropped — an unmapped `apiShape` now throws `LlmProxyUnroutableModelError` (500 `model_not_routable`) at the proxy and a clear `Error` at the run launcher, so a misconfigured preset can't silently bypass the gateway. Per-adapter `substituteModel()` wrappers were also dropped — the shared `substituteModelJson()` lives in `services/llm-proxy/helpers.ts` and is called directly from the core pipeline.
- ✅ **Phase 4a — response cache (opt-in, Appstrate-owned)** shipped. Two env vars: `PORTKEY_CACHE_MODE` (`off` | `simple` | `semantic`, default `off`) and `PORTKEY_CACHE_MAX_AGE` (seconds, default 3600). **Implementation note**: Portkey 1.15.2 OSS exposes a `cache: { mode }` contract in inline config but its standalone `start-server.js` bundle never installs the `getFromCache`/`putInCache` middleware — every gateway response comes back `cacheStatus: DISABLED` regardless of mode. So Appstrate mints the cache in `apps/api/src/services/llm-proxy/response-cache.ts`, in front of the upstream call, and emits a Portkey-compatible `x-portkey-cache-status: HIT | MISS` header so observability stays uniform. Key shape: `sha256(orgId + presetId + apiShape + realModelId + requestBody)` — cross-org / cross-preset requests never collide. Scope: non-streaming JSON, 2xx only (streaming SSE bypasses; 4xx/5xx never cached). Backend: Redis when `REDIS_URL` is set, in-memory per-process otherwise (via `infra/getCache()`). `semantic` mode is downgraded to `simple` at boot with a warning — proper embedding-store wiring is reserved for a follow-up. The Portkey inline `cache` field is still emitted in routing config for forward-compatibility once upstream fixes the bundled middleware. Integration tests: `apps/api/test/integration/routes/llm-proxy.test.ts` "response cache (#437 Phase 4a)" describe block (5 tests).
- ✅ **Phase 3 — open model catalog UX** shipped. Audit found the migration was already done in `0020_model_provider_credentials.sql` (free-text `provider_id text NOT NULL`, no enum), no Zod enum mirroring on the API surface, and `provider-registry-helpers.ts` was already driving the frontend entirely from `useProvidersRegistry()` with a `CUSTOM_ID` escape hatch. Only the featured/advanced UX split remained — added `featured?: boolean` to `ModelProviderDefinition` (core/module.ts) + `ProviderRegistryEntry` (shared-types), passed through the `/api/model-provider-credentials/registry` response, marked anthropic / openai / google-ai / mistral / codex as featured, and rendered `<SelectGroup>` sections in both the model-form and credential-form modals (featured first, "Other providers" below, Custom escape hatch at the bottom). The flag is advisory metadata only — any registry entry remains selectable from either group; no write path gates on it.
- ✅ **Phase 5 — single-source cost (drop inline duplicates)** shipped. Stripped `cost: { … }` literals from `core-providers/index.ts` for the 11 catalog-covered models (anthropic claude-\*, mistral, openai gpt-5\*/o4-mini). The registry endpoint now fills `cost` via `lookupModelCost(providerId, modelId)`. Eliminates the latent drift between form-UI cost and run-time cost that existed during Phases 2–4.
- ✅ **Phase 6 — catalog as single source of truth + full Portkey-routable surface** shipped. Re-vendored the catalog from `BerriAI/litellm` (MIT) instead of `Portkey-AI/models` — LiteLLM ships both pricing AND metadata (`max_input_tokens`, `supports_vision`, `mode`, …) in one file, while Portkey upstream only carries pricing. The catalog now covers 7 providers / 250 chat models (was 4 / 200, pricing-only). `pricing-catalog.ts` exposes `listCatalogModels(providerId)` + `lookupCatalogModel(providerId, modelId)` keyed on **providerId** (no longer apiShape — cerebras/groq/xai all share `openai-completions` apiShape with different upstreams). The picker now surfaces every catalog model: inline `models[]` ids in `core-providers` become the "Featured" curation, the rest of the catalog falls under "All models". Net `core-providers/index.ts` shrinks ~150 lines as inline metadata (contextWindow/maxTokens/capabilities) becomes catalog-derived. **Positioning shift**: Appstrate is now a marketplace of every Portkey-routable model, not just 30 curated ones. Adding a new provider's models = waiting for LiteLLM to ingest them (weekly upstream refresh + our cron PR). Drift caught at migration: gpt-5-mini was inline at \$0.75/\$4.50 but the canonical LiteLLM price is \$0.25/\$2.00 — the duplicated inline data was already stale, proof the centralization was overdue.
- **Phase 4b — fallback chains** — multi-provider failover via `targets[]` in the inline config (e.g. anthropic → bedrock on rate-limit). Requires a UI/data model for "primary + fallback model preset" pair; needs design work before scoping.
- **Phase 4c — OTel metrics** — **NOT shipping** in Portkey. Bundle scan confirms zero OTel / Prometheus surface in 1.15.2 OSS (the metrics export lives in Portkey Cloud). Would require a fork; out of scope. LiteLLM does have native OTel — if observability becomes a hard requirement, the cleanest path is a Phase 7 gateway swap, not a Portkey fork.
