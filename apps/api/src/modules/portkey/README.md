...
- ✅ **Phase 4a — response cache (opt-in)** shipped. Two env vars: `PORTKEY_CACHE_MODE` (`off` | `simple` | `semantic`, default `off`) and `PORTKEY_CACHE_MAX_AGE` (seconds, default 3600). Wired through `buildPortkeyRouting` so both router slots emit `cache: { mode, max_age }` in the inline `x-portkey-config`. `simple` mode (exact request hashing, process-local in-memory) works out of the box on 1.15.2 OSS. `semantic` mode requires a vector store and is exposed as a knob without end-to-end OSS validation — flag as experimental until we add a Redis vector backend wiring.
