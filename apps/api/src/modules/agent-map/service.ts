 ...
 * dropped.
 */
function routeDiagnostic(field: string): {
  nodeId: string | null;
  itemId: string | null;
} {
  ...
  if (field === "config") return { nodeId: "input_values", itemId: null };
  const configKey = field.match(/^config\.(.+)$/);
  if (configKey) return { nodeId: "input_values", itemId: configKey[1]! };
...
function memoryLabel(content: unknown, fallback: string): string {
  const value = typeof content === "string" ? content.trim() : JSON.stringify(content);
...
export async function buildAgentMap(
  ...
  const inheritedProxy = orgProxyList.find((proxy) => proxy.is_default && proxy.enabled) ?? null;
  const explicitDirect = packageConfig.proxyId === "none";
  const resolvedProxy = explicitDirect
    ? null
    : packageConfig.proxyId
      ? (orgProxyList.find((proxy) => proxy.id === packageConfig.proxyId) ?? null)
      : inheritedProxy;
  ...
  const leftCards: ColumnCard[] = [
    ...
    },
    {
      node: {
        ...
        type: "proxy",
        data: {
          proxy_id: explicitDirect ? "none" : (resolvedProxy?.id ?? null),
          proxy_name: explicitDirect ? null : (resolvedProxy?.label ?? null),
          inherited: packageConfig.proxyId === null,
          resolved: explicitDirect || resolvedProxy !== null,
        },
      },
      itemCount: 1,
    },
  ...
  const rightCards: ColumnCard[] = [
    ...
    },
    {
      node: {
        ...
        type: "memory",
        data: {
          available: canReadPersistence,
          items: [
            ...pinnedSlots.map((slot) => ({
              id: String(slot.id),
              label: slot.key,
              kind: "pinned",
              updated_at: slot.updatedAt?.toISOString() ?? null,
            })),
            ...memories.map((memory) => ({
              id: String(memory.id),
              label: memoryLabel(memory.content, `#${memory.id}`),
              kind: memory.pinned ? "pinned" : "archive",
              updated_at: memory.createdAt?.toISOString() ?? null,
            })),
          ],
        },
      },
      itemCount: pinnedSlots.length + memories.length,
    },
  ];
  ...
  // The edge inventory mirrors the product vocabulary instead of treating
  // every outer card as one generic environment relation.
  const edges: AgentMapEdge[] = [
    ...["schedules", "model", "proxy"].map((id) => ({
      id: `resolution-${id}`,
      source: id,
      target: "agent",
    ...
    ...rightCards
      .filter((c) => c.node.id !== "memory")
      .map((c) => ({
        id: `dependency-${c.node.id}`,
        source: c.node.id,
        target: "agent",
