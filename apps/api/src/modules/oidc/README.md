...
| `scopesToPermissions`                                                                                                                         | OAuth scope → core `Permission` set filter, polymorphic on `actor_type`. Identity scopes (`openid`/`profile`/`email`/`offline_access`) drop silently; on an end-user token the values in `OIDC_ALLOWED_SCOPES` (e.g. `agents:run`, `runs:read`) pass through verbatim; on a dashboard token the ceiling is the subject's live org role, which is what makes `runs:read-all` reachable there and nowhere else. Everything else is dropped with a debug log. |
