...
/**
 * Files API — gallery listing, metadata, content download, delete.
 *
 *   GET    /api/files             → list (files:read, actor-filtered)
 *   GET    /api/files/:id         → metadata DTO (+ derived `downloadable`)
 *   GET    /api/files/:id/content → 307 → presigned GET, or proxy-stream
 *   DELETE /api/files/:id         → delete (files:delete perm OR creator)
 *
 * Authorization is TWO layers, and both are load-bearing:
 *
 *   1. `files:read` (this file) — "may this principal touch the files
...
import { zFilePurposeEnum } from "@appstrate/db/schema";
import {
  getFileForActor,
  listFilesForActor,
  deleteFile,
...
export function createFilesRouter() {
  ...
  router.get("/files", rateLimit(120), requirePermission("files", "read"), async (c) => {
    ...
    const searchResult = fileSearchSchema.safeParse(c.req.query("q"));
  ...
  router.post("/files/:id/keep", rateLimit(60), async (c) => {
    ...
    const updated = await clearFileExpiry(scope, row.id);
    // Only audit an actual change (a no-op pin of an already-permanent file
 ...
 *    file id. No session is read; a session WITHOUT a token is a 401.
...
export function createFilePreviewRouter() {
  ...
  router.get("/preview/files/:id", rateLimitByIp(120), async (c) => {
    ...
    // The token authorizes exactly ONE file — reject a token minted for a
    // different id replayed on this path.
    if (payload.d !== c.req.param("id")) throw unauthorized("Preview token does not match file");

    const row = await loadFileForPreview(payload.o, payload.d);
    ...
    // minting actor is the file's creator — even a hand-crafted token that
    ...
    if (row.purpose === "user_upload") {
      ...
      if (!tokenActor || !getFileCapabilities(row, tokenActor, { visible: true }).download) {
        throw unauthorized("Preview token does not authorize this file");
      }
    }
