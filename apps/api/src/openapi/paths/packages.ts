...
export const packagesPaths = {
  ...
  "/api/packages/{scope}/{name}/{version}/download": {
    get: {
      ...
      description:
        "Download a specific version of a package as a ZIP file. Supports exact version, dist-tag, or semver range resolution. The literal `draft` downloads the author's working copy instead — the same tree `GET /api/packages/{scope}/{name}/files?version=draft` lists (the stored draft archive overlaid with the authoritative `manifest.json` and primary content from the database), zipped. Naming the draft is an author's act: it is reserved to callers who may WRITE the package (`403 draft_not_writable` otherwise, a system package included), on top of the visibility, `<type>:read` and `restrict_package_copy` checks every download applies. A draft archive has no integrity hash, so it carries no `X-Integrity`; it carries a strong `ETag` instead and answers `If-None-Match` with `304`. Rate-limited to 50 requests/minute.",
      parameters: [
        { $ref: "#/components/parameters/XOrgId" },
        { $ref: "#/components/parameters/XSpaceId" },
        { $ref: "#/components/parameters/PackageScope" },
  ...
  "/api/packages/{scope}/{name}/home": {
    ...
    put: {
      ...
      responses: {
        ...
        "403": { $ref: "#/components/responses/Forbidden" },
        "404": { $ref: "#/components/responses/NotFound" },
        // Ordered AFTER the 404 on purpose: a destination the caller cannot
        // reach is never confirmed to exist, so the 409 is only ever seen for
        // the caller's OWN personal space.
        "409": {
          description:
            "The destination is a personal space (`home_move_into_personal_space`). A personal space homes only what is created or forked in it — fork the package for a private copy. RFC 9457 problem+json.",
          content: {
            "application/problem+json": {
              schema: { $ref: "#/components/schemas/ProblemDetail" },
            },
