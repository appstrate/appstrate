...
/**
 * `0054_schema_integrity_repairs.sql` must actually repair the four defects it
 ...
 * the schema edits AND delete 0054 and it stays green, because the code and the
 * database would agree again — on the broken shape. It guards the INVARIANT.
 * This file guards the FIX, by modelling the population 0054 exists for and
 * checking that the shipped `.sql` converges it. Same split as
 * `migration-index-parity.test.ts` and `0041`.
 *
 * The pre-0054 state is built by UNDOING 0054 against a fully replayed database
 ...
 * A literal `DROP CONSTRAINT "<declared name>"` in 0054 passes every assertion
...
const MIGRATION = `${MIGRATIONS_DIR}/0054_schema_integrity_repairs.sql`;
...
describe("0054 — schema integrity repairs", () => {
  ...
  it("drops the four columns nothing reads", async () => {
    await pg.exec(`
      ...
      ALTER TABLE "chat_messages" ADD COLUMN "format" text;
    `);
    // The dependent index goes with its column, which is why 0054 does not name
    ...
    const messages = await columnNames("chat_messages");
    expect(messages.has("parent_id")).toBe(false);
    expect(messages.has("format")).toBe(false);
  });
