...
/**
 * `scripts/migration/0021-space-assignments-spaceid-key.sql` on a private
...
const invitation = () =>
  value(`SELECT space_assignments AS v FROM org_invitations WHERE id = 'inv_0021'`);
const client = () =>
...
beforeAll(async () => {
  ...
  await pg.exec(`
    ...
    INSERT INTO org_invitations (id, token, email, org_id, role, status, expires_at, space_assignments)
      VALUES ('inv_0021', 'tok_0021', 'i-0021@example.com', '${ORG}', 'guest', 'pending',
              now() + interval '7 days',
    ...
    INSERT INTO org_invitations (id, token, email, org_id, role, status, expires_at)
      VALUES ('inv_0021_empty', 'tok_0021_empty', 'e-0021@example.com', '${ORG}', 'member', 'pending',
              now() + interval '7 days');
    INSERT INTO oauth_clients (id, client_id, name, level, referenced_org_id, signup_role, signup_space_assignments, redirect_uris)
      VALUES ('oac_0021', 'oauth_0021', 'Signup', 'org', '${ORG}', 'guest',
...
describe("scripts/migration/0021 — space assignments carry `spaceId`", () => {
  it("renames the key in both columns, keeping order and role keys, and is idempotent", async () => {
    ...
    expect(
      await value(`SELECT space_assignments AS v FROM org_invitations WHERE id = 'inv_0021_empty'`),
    ).toEqual([]);
