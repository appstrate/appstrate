...
/**
 * Storage-backend parity for the files lifecycle.
 *
 ...
 *     bytes straight from storage (`streamFileContent`), so it returns bytes
...
import { deleteFile } from "../../../src/services/files.ts";
...
describe("files storage parity — core flow (ambient backend)", () => {
  let ctx: TestContext;
  let scope: Scope;
  let userActor: Actor;
...
describeRequiresS3("files storage parity — S3 presigned posture", () => {
  let ctx: TestContext;
  let scope: Scope;
  ...
  it("MCP resources/read still returns the bytes on the presigned posture (307-regression guard)", async () => {
    ...
    const read = await buildFileResourceProvider(mcpCtx).read(
      `appfile://${published.row.id}`,
      {} as Parameters<ReturnType<typeof buildFileResourceProvider>["read"]>[1],
    );
