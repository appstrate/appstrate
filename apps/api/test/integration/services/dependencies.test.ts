...
import { buildDependencies } from "../../../src/services/package-items/dependencies.ts";
...
describe("manifest-based dependency resolution", () => {
  ...
  describe("buildDependencies", () => {
    it("builds dependencies from manifest skills + tools + providers", async () => {
      ...
      const deps = await buildDependencies(`@${orgSlug}/my-flow`, orgId);
    ...
    it("returns null when package has no dependencies", async () => {
      ...
      const deps = await buildDependencies(`@${orgSlug}/no-deps`, orgId);
      expect(deps).toBeNull();
    });

    it("returns null for non-existent package", async () => {
      const deps = await buildDependencies("@nonexistent/pkg", orgId);
      expect(deps).toBeNull();
    });
  });
