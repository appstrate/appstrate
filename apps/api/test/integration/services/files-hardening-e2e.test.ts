...
/**
 * Cross-phase interaction tests for the files-hardening effort (tier0, FS
 * storage). The per-phase suites (files.test.ts, storage-deletion.test.ts,
...
import { files, organizations, runs, uploads, storageDeletionJobs } from "@appstrate/db/schema";
...
describe("files hardening — cross-phase interactions", () => {
  ...
  it("downgrade below used: list/download/delete keep working while new writes are refused, delete frees, write works again", async () => {
    ...
    await setOrgFileStorageLimit(ctx.orgId, 150);
    ...
    await deleteFile(scope, doc1!.id);
    expect(await orgBytesUsed(ctx.orgId)).toBe(100);
  ...
  it("deletion outbox drains a create+delete churn under a flaky deleteFile — every job eventually completes, nothing stranded", async () => {
    ...
    for (const c of created) await deleteFile(scope, c.row.id);
  ...
  it("two concurrent run-output publishes race the org limit — exactly one commits, the counter stays exact", async () => {
    ...
    // inside commitFileRow — the loser's re-check trips 403.
  ...
  it("same-named inputs keep distinct docs + unique workspace names; same-named outputs split by content, dedup on identical", async () => {
    ...
    const d1 = await createFileFromUpload(scope, userActor, u1, { runId });
    const d2 = await createFileFromUpload(scope, userActor, u2, { runId });
