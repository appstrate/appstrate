 ...
 * (claim extraction, `requiredIdentityClaims` gate) is covered in each
 * module's own test suite (under that module's `packages/module-<name>`
 * directory). This file uses the synthetic `test-oauth` provider so
 * removing or swapping a module never breaks core flow coverage.
 *
