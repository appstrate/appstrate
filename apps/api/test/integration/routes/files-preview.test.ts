...
/**
 * Integration tests for Phase 4 — the hardened, cookie-less file preview.
 *
 * The security assertions are the deliverable:
 *  - the preview route (`GET /preview/files/:id`) serves a file ONLY with
 ...
 *  - expired / cross-file / missing tokens 401; non-previewable / missing /
 ...
 *  - `GET /api/files/:id` mints `preview_url` + `preview_kind` for every
...
import { signPreviewToken, PREVIEW_MAX_BYTES } from "../../../src/services/file-preview.ts";
...
/** Seed a file row + write its bytes to the durable files bucket. */
...
describe("GET /preview/files/:id — hardened HTML preview", () => {
  let ctx: TestContext;
  ...
  it("rejects a valid session WITHOUT a token (no cookie auth fallback): 401", async () => {
    const docId = await seedDoc(ctx);
    const res = await app.request(`/preview/files/${docId}`, { headers: authHeaders(ctx) });
  ...
  it("rejects a token minted for file A replayed on file B: 401", async () => {
    const docA = await seedDoc(ctx);
    const docB = await seedDoc(ctx);
    const tokenA = mintToken(docA, ctx.orgId, nowSec() + 300);
    const res = await app.request(`/preview/files/${docB}?t=${encodeURIComponent(tokenA)}`);
    expect(res.status).toBe(401);
  });

  it("rejects a garbage token: 401", async () => {
    const docId = await seedDoc(ctx);
    const res = await app.request(`/preview/files/${docId}?t=not-a-real-token`);
    expect(res.status).toBe(401);
  });

  it("404s a non-previewable file even with a valid token", async () => {
  ...
  it("404s a deleted / unknown file (token cannot resurrect it)", async () => {
    const docId = await seedDoc(ctx);
    const token = mintToken(docId, ctx.orgId, nowSec() + 300);
    await db.delete(files).where(eq(files.id, docId));
    const res = await app.request(`/preview/files/${docId}?t=${encodeURIComponent(token)}`);
    expect(res.status).toBe(404);
  });

  it("404s a token whose org does not match the file's org", async () => {
    const docId = await seedDoc(ctx);
  ...
  it("413s a file larger than the preview cap", async () => {
  ...
  // preview is refused unless the token's bound minting actor is the file's
  ...
  describe("agent HTML is active ONLY in a nested frame — in every mode", () => {
    ...
    it("degrades a top-level load even when USERCONTENT_URL is configured", async () => {
      ...
      // file can still navigate itself and carry data out in the URL. So
      ...
      await withEnv("USERCONTENT_URL", "https://usercontent.example", async () => {
        ...
        for (const headers of headerSets) {
          ...
          expect(res.headers.get("content-security-policy")).not.toContain("unsafe-inline");
          expect(res.headers.get("content-security-policy")).not.toContain("sandbox");
          // Source, not a file: no meta CSP was injected.
          const body = await res.text();
          expect(body).toContain("<html>");
          expect(body).not.toContain('<meta http-equiv="Content-Security-Policy"');
        }
...
describe("GET /api/files/:id — preview_url minting", () => {
  let ctx: TestContext;
  ...
  it("mints preview_url + preview_kind=html for a text/html file, pointing at the preview route", async () => {
    const docId = await seedDoc(ctx, { mime: "text/html" });
    const res = await app.request(`/api/files/${docId}`, { headers: authHeaders(ctx) });
    expect(res.status).toBe(200);
    ...
    expect(url.pathname).toBe(`/preview/files/${docId}`);
  ...
  it("returns preview_url null + preview_kind null for a non-previewable file", async () => {
    const docId = await seedDoc(ctx, { mime: "application/octet-stream" });
    const res = await app.request(`/api/files/${docId}`, { headers: authHeaders(ctx) });
    const dto = (await res.json()) as {
      preview_url: string | null;
