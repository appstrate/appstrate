...
import { runs, files, organizations, auditEvents } from "@appstrate/db/schema";
...
function postDoc(runId: string, headers: Record<string, string>, body: Uint8Array | string) {
  return app.request(`/api/runs/${runId}/files`, { method: "POST", headers, body });
}

describe("POST /api/runs/:runId/files — agent-output ingestion", () => {
  let ctx: TestContext;
  ...
  it("stores a durable file: 201, row, org counter, sha256", async () => {
  ...
  it("dedups an identical (sha256, name) re-publish: 200, existing row, single count", async () => {
    ...
    const audits = await db
      .select()
      .from(auditEvents)
      .where(and(eq(auditEvents.action, "file.published"), eq(auditEvents.resourceId, firstId)));
  ...
  it("rejects a closed sink with 410 (before the handler runs)", async () => {
    ...
    // No file lands for a closed sink.
    const rows = await db.select().from(files).where(eq(files.runId, runId));
    expect(rows.length).toBe(0);
  });

  it("rejects a missing X-File-Name header with 400", async () => {
    const runId = await seedRun(ctx);
  ...
  it("IGNORES a legacy X-Document-Presentation header instead of rejecting it", async () => {
    ...
    expect(await db.select().from(files).where(eq(files.runId, runId))).toHaveLength(2);
  });
  ...
  it("rejects over the per-run file COUNT cap with a distinct 413", async () => {
    ...
    await withEnv("RUN_MAX_FILES", "2", async () => {
      ...
      // The third genuinely-new file exceeds the count cap.
      const res = await postDoc(runId, docHeaders(RUN_SECRET, "c.txt"), "ccc");
      expect(res.status).toBe(413);
      expect(((await res.json()) as { code: string }).code).toBe("file_count_exceeded");
      const rows = await db.select().from(files).where(eq(files.runId, runId));
  ...
  describe("authentication", () => {
    ...
    it("refuses a replayed webhook-id — one signed header set, one upload", async () => {
      ...
      // timestamp tolerance — each one a new file (distinct sha256 defeats
      // the (run, sha256, name) dedup), spending the org quota and the run's
      // file budget. Claiming the webhook-id makes the header set
      ...
      const replay = await postDoc(
        runId,
        { ...captured, "X-File-Name": "smuggled.txt" },
        "attacker bytes",
      );
      ...
      // Exactly one file exists, and the quota moved exactly once.
      const rows = await db.select().from(files).where(eq(files.runId, runId));
