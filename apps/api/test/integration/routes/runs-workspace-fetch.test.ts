 ...
 *   - `GET /api/runs/:runId/files`         — the input-file manifest
 *   - `GET /api/runs/:runId/files/:name`   — one file, streamed
 *
 * The agent extracts the bundle to its workspace root and streams each input
 * file to `files/<name>` on disk, instead of relying on a
 ...
 * materialised (issue #549). Files are stored as individual objects (not
 ...
 * 404-unknown), and the file fetches (manifest 200 / 404, doc 200 / 404).
 */
 ...
 *  uniqueness assertion in `writeRunFilesManifest` — used to simulate a
 ...
 * uploadRunBundle, each file streamed in, then the manifest written once.
 * Mirrors the split that lets the platform stream files through without
...
async function seedWorkspace(
  ...
  upload: {
    bundle?: Buffer;
    files: { name: string; content: Buffer; workspaceName?: string }[];
  },
): Promise<void> {
  if (upload.bundle) await uploadRunBundle(runId, upload.bundle);
  if (upload.files.length > 0) {
    for (const doc of upload.files) {
      await streamRunFile(runId, doc.workspaceName ?? doc.name, new Response(doc.content).body!);
    }
...
describe("run-workspace storage round-trip", () => {
  ...
  it("uploads, downloads, and deletes the bundle + files + manifest", async () => {
    ...
    // Manifest enumerates the files with their sizes.
    ...
    // Each file is fetchable as its own streamed object.
    const reportStream = await downloadRunFileStream(runId, "report.txt");
    ...
    expect(await downloadRunFileStream(runId, "report.txt")).toBeNull();
  });

  it("uploads only a bundle when there are no files (no manifest)", async () => {
    const runId = `run_${crypto.randomUUID().replace(/-/g, "").slice(0, 16)}`;
    await seedWorkspace(runId, { bundle: Buffer.from("BUNDLE"), files: [] });
    expect(await downloadRunWorkspace(runId)).not.toBeNull();
    expect(await downloadRunFilesManifest(runId)).toBeNull();
    await deleteRunWorkspace(runId);
  });

  it("writes nothing with no bundle and no files", async () => {
    const runId = `run_${crypto.randomUUID().replace(/-/g, "").slice(0, 16)}`;
    await seedWorkspace(runId, { files: [] });
    expect(await downloadRunWorkspace(runId)).toBeNull();
    expect(await downloadRunFilesManifest(runId)).toBeNull();
  });
  ...
  it("enqueues the whole teardown even when reading the manifest fails, and the worker finishes it", async () => {
    ...
    // (It used to download the manifest first to derive the file keys — a
    ...
    // retry — not a silent loss. The files survive this pass untouched.
    ...
    expect(await downloadRunFileStream(runId, "a.txt")).not.toBeNull();
    ...
    expect(await downloadRunWorkspace(runId)).toBeNull();
    expect(await downloadRunFilesManifest(runId)).toBeNull();
    expect(await downloadRunFileStream(runId, "a.txt")).toBeNull();
    expect(await downloadRunFileStream(runId, "b.txt")).toBeNull();
  });
});
...
describe("GET /api/runs/:runId/files[/:name]", () => {
  let ctx: TestContext;
  ...
  it("returns the manifest then streams each file", async () => {
    const runId = await seedRunWithSink(ctx, "@test/docs-agent");
    await seedWorkspace(runId, {
      bundle: Buffer.from("BUNDLE"),
      files: [{ name: "a.txt", content: Buffer.from("doc-a") }],
    });

    const manifestRes = await app.request(`/api/runs/${runId}/files`, {
      method: "GET",
      headers: signedGetHeaders(RUN_SECRET),
    });
  ...
  it("rejects a malformed manifest with duplicate workspace names (400 duplicate_file_name)", async () => {
    const runId = await seedRunWithSink(ctx, "@test/docs-agent");
    // A corrupted / hand-built manifest whose two entries resolve to the SAME
    // workspace_name would silently overwrite one file with the other in the
    ...
    expect(((await res.json()) as { code: string }).code).toBe("duplicate_file_name");

    await deleteRunWorkspace(runId);
  });

  it("returns 404 on the manifest when the run carries no files", async () => {
    const runId = await seedRunWithSink(ctx, "@test/docs-agent");
    await seedWorkspace(runId, { bundle: Buffer.from("BUNDLE"), files: [] });
  ...
  it("returns 404 for a file the run does not have", async () => {
    const runId = await seedRunWithSink(ctx, "@test/docs-agent");
    await seedWorkspace(runId, {
      bundle: Buffer.from("BUNDLE"),
      files: [{ name: "a.txt", content: Buffer.from("doc-a") }],
    ...
    const res = await app.request(`/api/runs/${runId}/files/missing.txt`, {
      method: "GET",
      headers: signedGetHeaders(RUN_SECRET),
    });
    expect(res.status).toBe(404);
  ...
  it("rejects an invalid signature with 401", async () => {
    ...
    const res = await app.request(`/api/runs/${runId}/files/a.txt`, {
      method: "GET",
      headers: signedGetHeaders("wrong-secret-".repeat(3)),
    });
    expect(res.status).toBe(401);
