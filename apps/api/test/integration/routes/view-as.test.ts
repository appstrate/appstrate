...
/**
 * "View as role" — the server half (docs/architecture/RBAC_VIEW_AS_PLAN.md §7).
 *
...
describe("view as role", () => {
  ...
  describe("orgRole readers", () => {
    /**
     * Every file that reads `c.get("orgRole")`, with why reading the REAL role
     * there is correct. A new reader fails this test on purpose: it must be
     * reviewed against the persona before it ships, because `orgRole` is the one
     * key a preview deliberately leaves untouched.
     */
    const ALLOWLIST: ReadonlyArray<[string, string]> = [
      [
      ...
      [
        ...
        "isFirstParty is behind `oauth-clients:write`, admin-tier and org-level, so no persona reaches it",
      ],
      [
        "apps/api/src/routes/organizations.ts",
        "membership gates plus who-manages-whom, each behind an admin-tier `members:*` guard",
      ],
      [
    ...
    it("is read only where the real role is what the site means", async () => {
      const root = `${import.meta.dir}/../../../../..`;
      // Whitespace-tolerant AND alias-tolerant: prettier may wrap the call
      // across lines, and `space-context.ts` reaches the context through a
      // `ctx` alias. A `c.`-anchored, line-oriented match would miss both.
      const reader = /\.get\(\s*"orgRole"\s*\)/;
      const found: string[] = [];
      for (const area of ["apps/api/src", "packages"]) {
        ...
        for await (const relative of new Bun.Glob(glob).scan({ cwd: `${root}/${area}` })) {
          ...
          if (reader.test(await Bun.file(`${root}/${path}`).text())) found.push(path);
        }
      }
      // Positive control: a regex that matched nothing would pass an empty
      // allowlist just as happily.
      expect(found).toContain("apps/api/src/lib/auth-pipeline.ts");
      expect(found.sort()).toEqual(ALLOWLIST.map(([file]) => file).sort());
    });
  ...
  describe("chat in-process loopback", () => {
    ...
    it("mints with the persona's org role, so a lost claim fails closed", async () => {
      // Installed in no space: only someone who manages the ORG catalog sees it.
      await seedAgent({ id: "@view-as/uninstalled", orgId: owner.orgId });
