 ...
 *
 * A launch form that seeds or submits the wrong values is not a cosmetic bug:
 * sending a locked field builds a request the server refuses with 400
 * `locked_input_field`, and echoing a resolved value back freezes it onto the
 * caller's layer. These tests pin the seed, the subset and the submitted delta
 * against the server's layers. The field partition and the layer-1+2 overlay
 * are core's (`packages/core/test/input-resolution.test.ts`).
 */

import { describe, it, expect } from "bun:test";
import type { SchemaWrapper } from "@appstrate/core/form";
import { resolvedInputDefaults, type AgentInputSettings } from "@appstrate/core/input-resolution";
import {
  formatInputValue,
  hasInputFields,
  initialInputValues,
