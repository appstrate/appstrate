...
export interface components {
    schemas: {
        ...
        Run: {
            ...
            /** @description Model context window (tokens) the run was LAUNCHED with — the denominator of the run's context gauge. The numerator is `contextTokens`, reported per turn on the run's `appstrate.progress` log breadcrumbs. Snapshotted at creation, so it stays correct after the org's model configuration changes. `null` means unknown — a run created before this field existed, a remote-origin run that resolves no platform model (it executes on the caller's host with the caller's own model), or a run whose model declares no context window (the platform does not guess one, because the runtime's own fallback differs from any value it could invent). Render no gauge on `null`; a zeroed gauge would read as 'context empty'. */
