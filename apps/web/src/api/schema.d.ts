...
export interface paths {
    ...
    "/api/integrations/{packageId}/auths/{authKey}/connect/fields": {
        ...
        get?: never;
        put?: never;
        /** Connect an api_key / basic / custom integration auth */
        post: operations["connectIntegrationFields"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
    ...
    "/api/mcp/o/{org}": {
         ...
         * @description Model Context Protocol server (Streamable HTTP, stateless) for a single organization. Accepts JSON-RPC 2.0 messages (`initialize`, `tools/list`, `tools/call`). Exposes three tools — `search_operations`, `describe_operation`, `invoke_operation` — that let an MCP client discover and call any platform API operation with the caller's own credentials, confined to the organization in the path. Each organization has its own endpoint: a token obtained for this endpoint is audience-bound (RFC 8707) to the per-org resource URI `<APP_URL>/api/mcp/o/{org}` and cannot drive any other organization. To use several organizations, configure one MCP server entry per organization. Requires the `mcp:read` permission (and `mcp:invoke` to call operations).
         */
    ...
    "/api/uploads/_content": {
        ...
        get?: never;
        /**
         * Write upload bytes (filesystem sink)
         * @description Public endpoint used when the platform runs in filesystem storage mode. Authenticated via HMAC-signed `token` query parameter returned in the upload descriptor. The token binds storage key, declared MIME, max size and expiry; the max size is enforced while the body streams to disk, so chunked uploads cannot exceed it. In S3 mode the signed URL points directly at the bucket and this endpoint is not used. Rate-limited per IP (60/min).
         */
