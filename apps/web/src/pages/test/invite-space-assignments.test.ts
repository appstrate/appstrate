...
const ONE = [{ spaceId: "spc_1", preset_role: "operator" as const }];
...
describe("toSpaceAssignments", () => {
  it("maps a preset row to `preset_role` and a custom row to `custom_role_id`", () => {
    expect(
      toSpaceAssignments([
        { spaceId: "spc_1", role: "preset:builder" },
        { spaceId: "spc_2", role: "custom:srl_abc" },
      ]),
    ).toEqual([
      { spaceId: "spc_1", preset_role: "builder" },
      { spaceId: "spc_2", custom_role_id: "srl_abc" },
    ]);
  });

  it("drops rows that are not yet complete", () => {
    expect(toSpaceAssignments([{ spaceId: "spc_1", role: "" }])).toEqual([]);
  });
});
