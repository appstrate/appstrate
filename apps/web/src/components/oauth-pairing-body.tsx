...
export function OAuthPairingBody({ providerId, onConnected, onBusyChange }: OAuthPairingBodyProps) {
  ...
  // and credential-list invalidation — are owned by
  // `<PendingPairingsWatcher>` so they fire even when this modal was already
  // closed. Deferred via queueMicrotask so the parent's setState-on-close
  // doesn't tear into this effect.
  useEffect(() => {
  ...
  async function generatePairing() {
    try {
      ...
      // Only non-secret fields are stored — never the command/token.
      const orgId = getCurrentOrgId();
      if (orgId) addPendingPairing({ id: res.id, providerId, expiresAt: res.expiresAt, orgId });
    } catch (err) {
      toast.error(
