...
import { OrgProfileProvidersBlock } from "../org-profile-providers-block";
...
export function FlowProvidersSection({ packageId }: { packageId: string }) {
  ...
  // Split providers: org-bound vs user-managed
  const orgProfileId = detail.forcedOrgProfileId;
  const orgProfileName = detail.forcedOrgProfileName;
  const orgBoundProviders = detail.dependencies.providers.filter((s) => s.source === "org_binding");
  const userProviders = detail.dependencies.providers.filter((s) => s.source !== "org_binding");

  const summary = computeProvidersSummary(userProviders, t);

  return (
    <>
      {/* Org profile block — shown when admin has configured an org profile */}
      {orgProfileId && orgProfileName && orgBoundProviders.length > 0 && (
        <OrgProfileProvidersBlock
          orgProfileId={orgProfileId}
          orgProfileName={orgProfileName}
          providerIds={orgBoundProviders.map((s) => s.id)}
          readOnly={!isOrgAdmin}
        />
      )}

      {/* User providers — only those not covered by the org profile */}
